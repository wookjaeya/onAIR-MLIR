module {
  ...
  hal.executable private @infer_dispatch_1 {
    hal.executable.variant public @embedded_elf_x86_64 target(<"llvm-cpu", "embedded-elf-x86_64", {cpu = "cascadelake", cpu_features = "+64bit,+adx,+aes,-amx-avx512,-amx-bf16,-amx-complex,-amx-fp16,-amx-fp8,-amx-int8,-amx-movrs,-amx-tf32,-amx-tile,+avx,-avx10.1,-avx10.2,+avx2,-avx512bf16,-avx512bitalg,+avx512bw,+avx512cd,+avx512dq,+avx512f,-avx512fp16,-avx512ifma,-avx512vbmi,-avx512vbmi2,+avx512vl,+avx512vnni,-avx512vp2intersect,-avx512vpopcntdq,-avxifma,-avxneconvert,-avxvnni,-avxvnniint16,-avxvnniint8,+bmi,+bmi2,-ccmp,-cf,-cldemote,+clflushopt,+clwb,-clzero,+cmov,-cmpccxadd,+crc32,+cx16,+cx8,-egpr,-enqcmd,+f16c,+fma,-fma4,+fsgsbase,+fxsr,-gfni,-hreset,+invpcid,-kl,-lwp,+lzcnt,+mmx,+movbe,-movdir64b,-movdiri,-movrs,-mwaitx,-ndd,-nf,+pclmul,-pconfig,-pku,+popcnt,-ppx,-prefetchi,+prfchw,-ptwrite,-push2pop2,-raoint,-rdpid,-rdpru,+rdrnd,+rdseed,+rtm,+sahf,-serialize,-sgx,-sha,-sha512,-shstk,-sm3,-sm4,+sse,+sse2,+sse3,+sse4.1,+sse4.2,-sse4a,+ssse3,-tbm,-tsxldtrk,-uintr,-usermsr,-vaes,-vpclmulqdq,-waitpkg,-wbnoinvd,-widekl,-xop,+xsave,+xsavec,+xsaveopt,+xsaves,-zu", data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, max_stack_allocation_size = 32768 : i64, native_vector_size = 64 : i64, target_triple = "x86_64-unknown-unknown-eabi-elf"}>) {
      ...
      builtin.module {
        func.func @infer_dispatch_1_conv_4x4x8x3x3x4_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert>} {
          ...
          %6 = linalg.fill {lowering_config = #iree_cpu.lowering_config<vector_common_parallel = [1, 1, 8]>} ins(%cst : f32) outs(%5 : tensor<4x4x8xf32>) -> tensor<4x4x8xf32>
          %7 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3, d4, d5) -> (d0 + d3, d1 + d4, d5)>, affine_map<(d0, d1, d2, d3, d4, d5) -> (d3, d4, d5, d2)>, affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%3, %4 : tensor<6x6x4xf32>, tensor<3x3x4x8xf32>) outs(%6 : tensor<4x4x8xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<distribution = [1, 1, 8, 0, 0, 0], vector_common_parallel = [1, 1, 8, 0, 0, 0], vector_reduction = [0, 0, 0, 1, 1, 16]>} {
          ^bb0(%in: f32, %in_0: f32, %out: f32):
            %9 = arith.mulf %in, %in_0 : f32
            %10 = arith.addf %out, %9 : f32
            linalg.yield %10 : f32
          } -> tensor<4x4x8xf32>
          %8 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%7 : tensor<4x4x8xf32>) outs(%5 : tensor<4x4x8xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<vector_common_parallel = [1, 1, 8]>} {
          ^bb0(%in: f32, %out: f32):
  ...
  util.global private mutable @infer_dispatch_1_embedded_elf_x86_64_infer_dispatch_1_conv_4x4x8x3x3x4_f32_buffer : !hal.buffer
  util.initializer {
    ...
    %buffer = hal.allocator.allocate<%allocator : !hal.allocator> affinity(%queue_affinity) type(%memory_type) usage(%buffer_usage) : !hal.buffer{%c4864}
    util.global.store %buffer, @infer_dispatch_1_embedded_elf_x86_64_infer_dispatch_1_conv_4x4x8x3x3x4_f32_buffer : !hal.buffer
    util.return
  }
  util.func public @infer_dispatch_1_embedded_elf_x86_64_infer_dispatch_1_conv_4x4x8x3x3x4_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    %0 = arith.index_cast %arg0 : i32 to index
    %device, %queue_affinity = hal.device.resolve on(#hal.device.affinity<@__device_0>) : !hal.device, i64
    %cmd = hal.command_buffer.create device(%device : !hal.device) mode("OneShot|AllowInlineExecution") categories(Dispatch) affinity(%queue_affinity) : !hal.command_buffer
    %infer_dispatch_1_embedded_elf_x86_64_infer_dispatch_1_conv_4x4x8x3x3x4_f32_buffer = util.global.load @infer_dispatch_1_embedded_elf_x86_64_infer_dispatch_1_conv_4x4x8x3x3x4_f32_buffer : !hal.buffer
    %c0 = arith.constant 0 : index
    %c1088 = arith.constant 1088 : index
    %c1280 = arith.constant 1280 : index
    %c2176 = arith.constant 2176 : index
    %c3584 = arith.constant 3584 : index
    %workgroup_x, %workgroup_y, %workgroup_z = hal.executable.calculate_workgroups device(%device : !hal.device) target(@infer_dispatch_1::@embedded_elf_x86_64::@infer_dispatch_1_conv_4x4x8x3x3x4_f32) : index, index, index
    %exe = hal.executable.lookup device(%device : !hal.device) executable(@infer_dispatch_1) : !hal.executable
    %ordinal = hal.executable.export.ordinal target(@infer_dispatch_1::@embedded_elf_x86_64::@infer_dispatch_1_conv_4x4x8x3x3x4_f32) : index
    %c1 = arith.constant 1 : index
    scf.for %arg1 = %c0 to %0 step %c1 {
      hal.command_buffer.dispatch<%cmd : !hal.command_buffer> target(%exe : !hal.executable)[%ordinal] workgroups([%workgroup_x, %workgroup_y, %workgroup_z]) bindings([
        (%infer_dispatch_1_embedded_elf_x86_64_infer_dispatch_1_conv_4x4x8x3x3x4_f32_buffer : !hal.buffer)[%c0, %c1088], 
        (%infer_dispatch_1_embedded_elf_x86_64_infer_dispatch_1_conv_4x4x8x3x3x4_f32_buffer : !hal.buffer)[%c1280, %c2176], 
        (%infer_dispatch_1_embedded_elf_x86_64_infer_dispatch_1_conv_4x4x8x3x3x4_f32_buffer : !hal.buffer)[%c3584, %c1088]
      ]) flags("None")
