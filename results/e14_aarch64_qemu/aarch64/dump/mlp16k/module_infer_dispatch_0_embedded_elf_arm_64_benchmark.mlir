module {
  ...
  hal.executable private @infer_dispatch_0 {
    hal.executable.variant public @embedded_elf_arm_64 target(<"llvm-cpu", "embedded-elf-arm_64", {cpu = "cortex-a53", cpu_features = "+v8a,+aes,+crc,+fp-armv8,+neon,+perfmon,+sha2,+reserve-x18", data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i8:8:32-i16:16:32-i64:64-i128:128-n32:64-S128-Fn32", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, max_stack_allocation_size = 32768 : i64, native_vector_size = 16 : i64, target_triple = "aarch64-unknown-unknown-eabi-elf"}>) {
      ...
      builtin.module {
        func.func @infer_dispatch_0_matmul_1x16384x9_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert, {enable_loop_peeling}>} {
          ...
          %6 = linalg.fill {lowering_config = #iree_cpu.lowering_config<cache_parallel = [0, 64], vector_common_parallel = [1, 8]>} ins(%cst : f32) outs(%5 : tensor<1x16384xf32>) -> tensor<1x16384xf32>
          %7 = linalg.matmul {lowering_config = #iree_cpu.lowering_config<cache_parallel = [0, 64, 0], distribution = [0, 64, 0], vector_common_parallel = [1, 8, 0], vector_reduction = [0, 0, 4]>} ins(%3, %4 : tensor<1x9xf32>, tensor<9x16384xf32>) outs(%6 : tensor<1x16384xf32>) -> tensor<1x16384xf32>
  ...
  util.global private mutable @infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_matmul_1x16384x9_f32_buffer : !hal.buffer
  util.initializer {
    %device, %queue_affinity = hal.device.resolve on(#hal.device.affinity<@__device_0>) : !hal.device, i64
    %allocator = hal.device.allocator<%device : !hal.device> : !hal.allocator
    %memory_type = hal.memory_type<"DeviceVisible|DeviceLocal"> : i32
    %buffer_usage = hal.buffer_usage<"TransferSource|TransferTarget|Transfer|DispatchStorageRead|DispatchStorageWrite|DispatchStorage"> : i32
    %c786688 = arith.constant 786688 : index
    %buffer = hal.allocator.allocate<%allocator : !hal.allocator> affinity(%queue_affinity) type(%memory_type) usage(%buffer_usage) : !hal.buffer{%c786688}
    util.global.store %buffer, @infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_matmul_1x16384x9_f32_buffer : !hal.buffer
    util.return
  }
  util.func public @infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_matmul_1x16384x9_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    %0 = arith.index_cast %arg0 : i32 to index
    %device, %queue_affinity = hal.device.resolve on(#hal.device.affinity<@__device_0>) : !hal.device, i64
    %cmd = hal.command_buffer.create device(%device : !hal.device) mode("OneShot|AllowInlineExecution") categories(Dispatch) affinity(%queue_affinity) : !hal.command_buffer
    %infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_matmul_1x16384x9_f32_buffer = util.global.load @infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_matmul_1x16384x9_f32_buffer : !hal.buffer
    %c0 = arith.constant 0 : index
    %c36 = arith.constant 36 : index
    %c256 = arith.constant 256 : index
    %c720896 = arith.constant 720896 : index
    %c721152 = arith.constant 721152 : index
    %c65536 = arith.constant 65536 : index
    %workgroup_x, %workgroup_y, %workgroup_z = hal.executable.calculate_workgroups device(%device : !hal.device) target(@infer_dispatch_0::@embedded_elf_arm_64::@infer_dispatch_0_matmul_1x16384x9_f32) : index, index, index
    %exe = hal.executable.lookup device(%device : !hal.device) executable(@infer_dispatch_0) : !hal.executable
    %ordinal = hal.executable.export.ordinal target(@infer_dispatch_0::@embedded_elf_arm_64::@infer_dispatch_0_matmul_1x16384x9_f32) : index
    %c1 = arith.constant 1 : index
    scf.for %arg1 = %c0 to %0 step %c1 {
      hal.command_buffer.dispatch<%cmd : !hal.command_buffer> target(%exe : !hal.executable)[%ordinal] workgroups([%workgroup_x, %workgroup_y, %workgroup_z]) bindings([
        (%infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_matmul_1x16384x9_f32_buffer : !hal.buffer)[%c0, %c36], 
        (%infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_matmul_1x16384x9_f32_buffer : !hal.buffer)[%c256, %c720896], 
        (%infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_matmul_1x16384x9_f32_buffer : !hal.buffer)[%c721152, %c65536]
      ]) flags("None")
