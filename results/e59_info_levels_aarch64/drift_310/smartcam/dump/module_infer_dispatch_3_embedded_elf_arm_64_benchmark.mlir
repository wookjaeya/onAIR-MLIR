module {
    ...
    hal.executable.variant public @embedded_elf_arm_64 target(<"llvm-cpu", "embedded-elf-arm_64", {cpu = "cortex-a53", cpu_features = "+v8a,+aes,+crc,+fp-armv8,+neon,+perfmon,+sha2,+reserve-x18", data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i8:8:32-i16:16:32-i64:64-i128:128-n32:64-S128-Fn32", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, max_stack_allocation_size = 32768 : i64, native_vector_size = 16 : i64, target_triple = "aarch64-unknown-unknown-eabi-elf"}>) {
      ...
      builtin.module {
        func.func @infer_dispatch_3_matmul_like_16x12544x32_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert, {enable_loop_peeling}>} {
          ...
          %6 = linalg.fill {lowering_config = #iree_cpu.lowering_config<cache_parallel = [16, 64], vector_common_parallel = [8, 16]>} ins(%cst : f32) outs(%5 : tensor<16x12544xf32>) -> tensor<16x12544xf32>
          ...
          %8 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%7, %cst_0 : tensor<16x12544xf32>, tensor<16xf32>) outs(%5 : tensor<16x12544xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<cache_parallel = [16, 64], vector_common_parallel = [8, 16]>} {
          ^bb0(%in: f32, %in_1: f32, %out: f32):
  ...
  util.global private mutable @infer_dispatch_3_embedded_elf_arm_64_infer_dispatch_3_matmul_like_16x12544x32_f32_buffer : !hal.buffer
  util.initializer {
    ...
    util.global.store %buffer, @infer_dispatch_3_embedded_elf_arm_64_infer_dispatch_3_matmul_like_16x12544x32_f32_buffer : !hal.buffer
    util.return
  }
  util.func public @infer_dispatch_3_embedded_elf_arm_64_infer_dispatch_3_matmul_like_16x12544x32_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    %0 = arith.index_cast %arg0 : i32 to index
    %device, %queue_affinity = hal.device.resolve on(#hal.device.affinity<@__device_0>) : !hal.device, i64
    %cmd = hal.command_buffer.create device(%device : !hal.device) mode("OneShot|AllowInlineExecution") categories(Dispatch) affinity(%queue_affinity) : !hal.command_buffer
    %infer_dispatch_3_embedded_elf_arm_64_infer_dispatch_3_matmul_like_16x12544x32_f32_buffer = util.global.load @infer_dispatch_3_embedded_elf_arm_64_infer_dispatch_3_matmul_like_16x12544x32_f32_buffer : !hal.buffer
    %c0 = arith.constant 0 : index
    ...
    %workgroup_x, %workgroup_y, %workgroup_z = hal.executable.calculate_workgroups device(%device : !hal.device) target(@infer_dispatch_3::@embedded_elf_arm_64::@infer_dispatch_3_matmul_like_16x12544x32_f32) : index, index, index
    %exe = hal.executable.lookup device(%device : !hal.device) executable(@infer_dispatch_3) : !hal.executable
    %ordinal = hal.executable.export.ordinal target(@infer_dispatch_3::@embedded_elf_arm_64::@infer_dispatch_3_matmul_like_16x12544x32_f32) : index
    %c1 = arith.constant 1 : index
    scf.for %arg1 = %c0 to %0 step %c1 {
      hal.command_buffer.dispatch<%cmd : !hal.command_buffer> target(%exe : !hal.executable)[%ordinal] workgroups([%workgroup_x, %workgroup_y, %workgroup_z]) bindings([
        (%infer_dispatch_3_embedded_elf_arm_64_infer_dispatch_3_matmul_like_16x12544x32_f32_buffer : !hal.buffer)[%c0, %c8779968], 
        (%infer_dispatch_3_embedded_elf_arm_64_infer_dispatch_3_matmul_like_16x12544x32_f32_buffer : !hal.buffer)[%c8780032, %c8840704], 
        (%infer_dispatch_3_embedded_elf_arm_64_infer_dispatch_3_matmul_like_16x12544x32_f32_buffer : !hal.buffer)[%c17620736, %c8779968]
      ]) flags("None")
