module {
  ...
  hal.executable private @infer_dispatch_5 {
    hal.executable.variant public @embedded_elf_arm_64 target(<"llvm-cpu", "embedded-elf-arm_64", {cpu = "cortex-a53", cpu_features = "+v8a,+aes,+crc,+fp-armv8,+neon,+perfmon,+sha2,+reserve-x18", data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i8:8:32-i16:16:32-i64:64-i128:128-n32:64-S128-Fn32", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, max_stack_allocation_size = 32768 : i64, native_vector_size = 16 : i64, target_triple = "aarch64-unknown-unknown-eabi-elf"}>) {
      ...
      builtin.module {
        func.func @infer_dispatch_5_conv_128x224x224x128x3x3_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert, {enable_loop_peeling}>} {
          ...
          %17 = linalg.fill {lowering_config = #iree_cpu.lowering_config<vector_common_parallel = [1, 1, 4]>} ins(%cst_0 : f32) outs(%16 : tensor<128x224x224xf32>) -> tensor<128x224x224xf32>
          %18 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3, d4, d5) -> (d3, d1 + d4, d2 + d5)>, affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d3, d4, d5)>, affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%13, %14 : tensor<128x226x226xf32>, tensor<128x128x3x3xf32>) outs(%17 : tensor<128x224x224xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<distribution = [32, 32, 32, 0, 0, 0], vector_common_parallel = [1, 1, 4, 0, 0, 0], vector_reduction = [0, 0, 0, 1, 1, 4]>} {
          ^bb0(%in: f32, %in_1: f32, %out: f32):
            %20 = arith.mulf %in, %in_1 : f32
            %21 = arith.addf %out, %20 : f32
            linalg.yield %21 : f32
          } -> tensor<128x224x224xf32>
          %19 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%18, %15 : tensor<128x224x224xf32>, tensor<128xf32>) outs(%16 : tensor<128x224x224xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<vector_common_parallel = [1, 1, 4]>} {
          ^bb0(%in: f32, %in_1: f32, %out: f32):
  ...
  util.global private mutable @infer_dispatch_5_embedded_elf_arm_64_infer_dispatch_5_conv_128x224x224x128x3x3_f32_buffer : !hal.buffer
  util.initializer {
    ...
    util.global.store %buffer, @infer_dispatch_5_embedded_elf_arm_64_infer_dispatch_5_conv_128x224x224x128x3x3_f32_buffer : !hal.buffer
    util.return
  }
  util.func public @infer_dispatch_5_embedded_elf_arm_64_infer_dispatch_5_conv_128x224x224x128x3x3_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    %0 = arith.index_cast %arg0 : i32 to index
    %device, %queue_affinity = hal.device.resolve on(#hal.device.affinity<@__device_0>) : !hal.device, i64
    %cmd = hal.command_buffer.create device(%device : !hal.device) mode("OneShot|AllowInlineExecution") categories(Dispatch) affinity(%queue_affinity) : !hal.command_buffer
    %c103566848_i32 = arith.constant 103566848 : i32
    %c885632_i32 = arith.constant 885632 : i32
    %c1475456_i32 = arith.constant 1475456 : i32
    %c32112640_i32 = arith.constant 32112640 : i32
    %infer_dispatch_5_embedded_elf_arm_64_infer_dispatch_5_conv_128x224x224x128x3x3_f32_buffer = util.global.load @infer_dispatch_5_embedded_elf_arm_64_infer_dispatch_5_conv_128x224x224x128x3x3_f32_buffer : !hal.buffer
    ...
    %workgroup_x, %workgroup_y, %workgroup_z = hal.executable.calculate_workgroups device(%device : !hal.device) target(@infer_dispatch_5::@embedded_elf_arm_64::@infer_dispatch_5_conv_128x224x224x128x3x3_f32) : index, index, index
    %exe = hal.executable.lookup device(%device : !hal.device) executable(@infer_dispatch_5) : !hal.executable
    %ordinal = hal.executable.export.ordinal target(@infer_dispatch_5::@embedded_elf_arm_64::@infer_dispatch_5_conv_128x224x224x128x3x3_f32) : index
    %c1 = arith.constant 1 : index
    scf.for %arg1 = %c0 to %0 step %c1 {
      hal.command_buffer.dispatch<%cmd : !hal.command_buffer> target(%exe : !hal.executable)[%ordinal] workgroups([%workgroup_x, %workgroup_y, %workgroup_z]) constants([%c103566848_i32, %c885632_i32, %c1475456_i32, %c32112640_i32]) bindings([
        (%infer_dispatch_5_embedded_elf_arm_64_infer_dispatch_5_conv_128x224x224x128x3x3_f32_buffer : !hal.buffer)[%c0, %c130178560], 
        (%infer_dispatch_5_embedded_elf_arm_64_infer_dispatch_5_conv_128x224x224x128x3x3_f32_buffer : !hal.buffer)[%c130178560, %c4283648], 
        (%infer_dispatch_5_embedded_elf_arm_64_infer_dispatch_5_conv_128x224x224x128x3x3_f32_buffer : !hal.buffer)[%c134462208, %c130178560]
      ]) flags("None")
