module {
  ...
  util.global private mutable @infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_slow_memcpy_buffer : !hal.buffer
  util.initializer {
    %device, %queue_affinity = hal.device.resolve on(#hal.device.affinity<@__device_0>) : !hal.device, i64
    %allocator = hal.device.allocator<%device : !hal.device> : !hal.allocator
    %memory_type = hal.memory_type<"DeviceVisible|DeviceLocal"> : i32
    %buffer_usage = hal.buffer_usage<"TransferSource|TransferTarget|Transfer|DispatchStorageRead|DispatchStorageWrite|DispatchStorage"> : i32
    %c309504 = arith.constant 309504 : index
    %buffer = hal.allocator.allocate<%allocator : !hal.allocator> affinity(%queue_affinity) type(%memory_type) usage(%buffer_usage) : !hal.buffer{%c309504}
    util.global.store %buffer, @infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_slow_memcpy_buffer : !hal.buffer
    util.return
  }
  util.func public @infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_slow_memcpy(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    %0 = arith.index_cast %arg0 : i32 to index
    %device, %queue_affinity = hal.device.resolve on(#hal.device.affinity<@__device_0>) : !hal.device, i64
    %cmd = hal.command_buffer.create device(%device : !hal.device) mode("OneShot|AllowInlineExecution") categories(Dispatch) affinity(%queue_affinity) : !hal.command_buffer
    %infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_slow_memcpy_buffer = util.global.load @infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_slow_memcpy_buffer : !hal.buffer
    %c0 = arith.constant 0 : index
    %c12288 = arith.constant 12288 : index
    %c297088 = arith.constant 297088 : index
    %workgroup_x, %workgroup_y, %workgroup_z = hal.executable.calculate_workgroups device(%device : !hal.device) target(@infer_dispatch_0::@embedded_elf_arm_64::@infer_dispatch_0_slow_memcpy) : index, index, index
    %exe = hal.executable.lookup device(%device : !hal.device) executable(@infer_dispatch_0) : !hal.executable
    %ordinal = hal.executable.export.ordinal target(@infer_dispatch_0::@embedded_elf_arm_64::@infer_dispatch_0_slow_memcpy) : index
    %c1 = arith.constant 1 : index
    scf.for %arg1 = %c0 to %0 step %c1 {
      hal.command_buffer.dispatch<%cmd : !hal.command_buffer> target(%exe : !hal.executable)[%ordinal] workgroups([%workgroup_x, %workgroup_y, %workgroup_z]) bindings([
        (%infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_slow_memcpy_buffer : !hal.buffer)[%c0, %c12288], 
        (%infer_dispatch_0_embedded_elf_arm_64_infer_dispatch_0_slow_memcpy_buffer : !hal.buffer)[%c12288, %c297088]
      ]) flags("None")
