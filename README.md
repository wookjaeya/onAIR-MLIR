...
**현재 버전**: v0.49 (`git tag` 대신 커밋 이력·[`CHANGELOG.md`](./CHANGELOG.md)로 확인 — **원격 저장소에는 태그가 하나도 없다**: `git ls-remote --tags origin`이 빈 결과이고, 이 실행 환경에서 태그 푸시는 403으로 거부된다. `v0.49`를 붙이려면 로컬 클론에서 `git tag -a v0.42 <main의 병합 커밋> && git push origin v0.42`를 직접 실행해야 한다).
