...
이 컨테이너 **508/508 → 539/539**. **CI 실측**(커밋 `6598cf4`, run 157, 3레그 success): `full` **538/538 + 1 SKIP**(PyYAML) ·
`without-iree` **398/398 + 26 SKIP** · `stdlib-only` **398/398 + 26 SKIP** — 컨테이너 539/539와 `full`의
차이 1건은 PyYAML 유무다(D34: 추정하지 않고 두 수치를 조건과 함께 병기). 축소 레그가 367→398로
**정확히 +31**이라 E37 신규 31건이 **전부 나타난다**(신규 SKIP 0): 보관 JSON 판독·소스 텍스트 검사·
게스트 raw log 판독이라 툴체인 없이 실제로 돈다.
...
남은 것은 정규 MLIR pass(미착수 — E27·E35 결과상 만들 이유가 아니라 주장을 좁힐 이유이며, *"만들어도 결과가
바뀌지 않음을 실측했다"*고는 **쓸 수 없다**. pass를 만들어 비교한 실험은 없다), WGAN 반입이다.
...
- *"MLIR 수준이 더 나은 admission 판정을 낸다"* → **E35가 24/24 동일로 반증했다**(4개 모델–타깃 구성 × 2정책 × 3예산구간, 판정 불일치 0)
...
- *"정규 pass를 만들어도 결과가 바뀌지 않음을 실측했다"* → pass를 구현해 비교한 실험은 **없다**. 쓸 수 있는 것은 *"현재 두 정보 수준에서 수치·판정 우위가 관측되지 않았다"*까지다
