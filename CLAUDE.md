...
- *"OnAIR 경로에서 출력 버퍼 해제를 검증했다"* → **D60으로 철회**. 원자료가 종료 시점 미해제 인스턴스를 보고한다. 반대로 *"누수가 있다"*도 쓸 수 없다 — 상태는 **미검증**이다 (**v0.66/E57 이후**: AArch64 게스트의 IREE 파이썬 휠 경로에서는 **호출마다 출력 버퍼 하나가 남는 것을 관측**했다 — D103. 쓸 수 있는 것은 *"이 휠·이 드라이버의 OnAIR 플러그인 경로에서 호스트 readback이 출력 버퍼를 호출마다 보유하고, DeepAE는 417회째에 승인 예산을 넘었다"*까지이고, IREE 상류 결함이라고는 쓰지 않는다)
