...
연구. 현재 버전: **v0.29**(git tag는 환경 제약으로 보류 — 커밋 이력·CHANGELOG로 확인).
...
수치가 아니라 독립적인 두 번째 정보원이며, fail-closed는 의지가 아니라 정보원 수의 문제다.**
...
이 컨테이너 **276/276**. **주장하지 않음**: 정규 MLIR pass(미착수), **(b) 실패는 n=1**,
(d)를 정답으로 삼는 서술.
...
**R-3. MLIR 접근의 고유 기여 (검토 C3 / E27)** — **완결(v0.29/E27)**. 답은
   *"MLIR이라야 이 수치를 얻는다"*가 아니라 **"MLIR 수준은 독립적인 두 번째 정보원을 준다"**이다.
   ...
   드리프트에서만 났다 — 거기서 그 분석기는 **읽지 못한 것을 없다고 보고**한다(152배 과소, 무경고).
   `docs/EVIDENCE_v0.29_E27.md` 참조. 아래는 착수 당시의 서술이며 이력으로 남긴다.
...
- **(R-3, v0.29/E27)** MLIR 수준 정보의 기여는 **더 정확한 수치가 아니라 독립적인 두 번째
  ...
  무경고). **fail-closed는 의지가 아니라 정보원 수의 문제다.**
...
docs/
  ...
  EVIDENCE_v0.29_E27.md       ★ 최신. R-3 답 — 정상 8/8은 (b)=(c), 차이는 교란에서만.
