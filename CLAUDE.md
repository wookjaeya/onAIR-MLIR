...
**논문은 둘 중 어느 쪽인지 명시해야 한다.** 참고: E24b의 R1은 **신뢰된 생성기 가정
아래에서도** 정상 파이프라인으로 도달했으므로, 어느 쪽을 택하든 그 수정은 유지된다.

**초안 작성됨 — `docs/THREAT_MODEL.md`(v0.21, 결정 대기).** 그 문서의 §1이 실측한 핵심
사실: **저장소는 두 경로에서 사실상 서로 다른 threat model로 동작한다.** cFS 앱과 native C는
계약 값을 `contract_gen.h`로 컴파일 타임에 받고 런타임에 계약 JSON을 읽지 않으며, 대상에
올라가는 것은 `ai_learner.so`와 `model.vmfb`뿐이다(계약은 빌드 시스템의 내부 중간물).
OnAIR 플러그인만 런타임에 계약 JSON을 읽는다(배포 산물). 또한 계약은 서명되지 않으므로
**provenance 필수화로는 위조를 막지 못한다**(E24c/F1 실측) — 막으려면 서명이 필요하고 그건
아키텍처 변경이다. 권고는 **선택지 A(신뢰된 생성기) + OnAIR 경로 예외 명시**이며, 논문
문장 초안이 §6에 있다. **결정은 연구자의 몫이므로 여기서 임의로 확정하지 않는다.**
...
docs/
  THREAT_MODEL.md             ★ 신뢰 경계 초안(v0.21, 결정 대기) — 배포 시점에 계약 JSON이
                               어디 있는지 실측(C 경로는 빌드 내부물, OnAIR만 배포 산물)
