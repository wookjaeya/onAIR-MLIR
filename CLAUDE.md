...
연구. 현재 버전: **v0.70**(git tag는 v0.9 이후 미부착 — 커밋 이력·CHANGELOG로 확인).
...
**선언 검사이지 "다른 driver가 위험하다"가 아니다** — N=1 HAL 피크는 `local-sync`/`local-task`에서 바이트 동일이다(**E41b/D100**: E41은 그 수치를 산문으로만 적고 셀을 남기지 않았다. 6/6 셀로 실측해 `results/e41b_driver_peak/`에 보관했고, **적재 분기를 드라이버 간에 확정하지는 않는다**).
...
**헤더가 생성되지 않는다**(**정정 v0.69/D106**: 이 관측은 하네스가 헤더 생성기를 잘못 불러 양성 대조군까지 rc 2였던 결과다 — 실제로는 헤더가 `CONTRACT_BOUND_KNOWN 0`으로 **생성되고** 비행 앱이 Init에서 `UNKNOWN_BOUND`로 거부한다)(세 번째가 미발행이 아닌 것은 설계대로다 — 막으면 E24가 N1에서 고친 과잉 거부가
...
**v0.54에서 완료된 것 (E52, `docs/EVIDENCE_v0.54_E52.md`)**: 검토 §4가 정한 **단계 4 — DeepAE 수치 불일치의
원인 분석**. 사전 고정 기준은 `docs/plans/E52_deepae_divergence.md`(커밋 `8ffef25`, **측정 이전**).
**§4.4의 첫 번째 종료 조건을 충족했다** — 최초 불일치 단계와 기술적 원인을 재현 가능한 근거로 확인했다.
**허용오차는 바뀌지 않았고 FAIL은 결과로 그대로 남는다**(D74). 모델이 `FULLY_CONNECTED` 10개뿐이고
(640→128×4→8→128×4→640, **마지막 층만 ReLU 없음**) 배포 linalg MLIR이 상수 20개를 dense로 싣고 있어
층 경계마다 잘라 컴파일할 수 있었다. **최초 불일치 L6**, 판정에 닿는 대량 실패는 **L9**(위반 94 =
E45·E48이 이 샘플에서 본 그 94). 레이어 분해를 믿는 근거를 **먼저** 세웠다 — 진단 체인의 마지막 모듈이
**배포 vmfb와 비트 동일**하고, LiteRT 중간값을 얻은 `preserve_all_tensors` 실행의 **최종 출력이 기본
실행과 비트 동일**하다.
**원인은 누산 순서다.** 네 후보 중 셋을 측정으로 배제했다 — **변환 의미**(배포 상수 **20/20이 원본
`.tflite`와 바이트 동일**) · **전처리**(E45 §2.3 — **D101 정정**, 이전 §2.4는 한 절 어긋났다) · **FMA**(`-fma`를 끄거나 generic CPU로 낮춰도 출력이
배포본과 **비트 동일**). 남은 하나는 이름을 붙일 수 있었다: **x86-64 IREE 출력이 float32 순차 누산
참조와 `worst_abs = 0.0`으로 640원소 전부 비트 동일**이고, LiteRT(↔float64 위반 85, 기본 delegate를
끄면 42)와 AArch64 IREE(↔순차 위반 30)는 각각 다르다 — **세 구현이 세 가지 누산 순서**. delegate는
**기여하지만 단독 원인이 아니다**.
**기전**: 오차가 L0→L9에서 **51.9배** 자라는데(ReLU가 음수를 자를 때 오차도 잘려 단조는 아니다) OR
규칙의 유효 문턱 `max(1e-4, 1e-5·|v|)`은 마지막 층만 ReLU가 없어 dB 규모(중앙값 |v| 24.65)가 돼도
**2.46배**만 오른다. **51.9가 2.46을 앞지른다** — E45의 *"값 규모가 원인의 큰 부분"*을 정밀화하면
규모 자체가 아니라 **누적 대 규모의 비**다.
...
**연구 책임자 결정 갱신 (2026-09-12, `docs/reviews/DECISIONS_v0_52_REVIEW_R2_WGAN.md`)**: 검토서
`DECISIONS_v0_52_REVIEW.md`의 개정판이 도착해 **WGAN AArch64 항목 하나만** 바꿨다(다른 아홉 항목은
원문 그대로 — 직접 diff로 확인). §10-5가 *"보류"*에서 **"핵심 검증 후 수행"**으로 바뀌었고, 순서
(§10-2)에 **"→ WGAN AArch64 종단 검증"**이 추가됐다. 사유(§7.2, 개정): WGAN의 호출당 메모리(`per_call`
131,382,784 B)가 AArch64에서 실행한 기존 최대 사례(SmartCam `per_call` 9,382,092 B)의 **14배**라
*"모델 하나를 늘리는 것"*이 아니라 *"큰 임시 버퍼에서 분석·admission이 유지되는지 보는 스트레스
사례"*다. 그 순서가 요구하는 선행조건(E50 연결·순차 호출·회계 매핑·DeepAE 원인)은 **이번 세션에서
전부 닫혔다**(E51 단계 1~3, E52) — 따라서 **WGAN AArch64 종단 검증이 지금 지시된 다음 작업**이다.
...
**v0.57에서 완료된 것 (E54, `docs/EVIDENCE_v0.57_E54.md`)**: **참조 기반 메모리 예산과 admission
매트릭스** — 연구 책임자 지시(`docs/reviews/BUDGET_BASED_ADMISSION_ANALYSIS.md`, 원문 보존 `c08d790`)가
*"모델과 독립적으로 산정된 배치 메모리 예산을 적용했을 때 네 실제 모델의 ADMIT/DENY가 어떻게 달라지는가"*를
남은 질문으로 못박았다. **지금까지의 모든 예산 셀은 계약값의 함수였다**(`B`·`B−1`·`per_call`) — 그것은 게이트의
경계 동작은 보이지만 *"실제 임무 자원 조건에서 이 모델들이 들어가는가"*에는 답하지 않는다. 사전 고정 기준
`docs/plans/E54_reference_budget_admission.md`(커밋 `8f128ae`, **측정 이전**).
**비순환성의 근거는 산문이 아니라 커밋 순서다**: 계획 → M2·M3 기준선+출처(`18dfac1`, **모델 없음·예산 없음**)
→ `budgets.json`(`25df44b`, **셀 실행 이전**) → 시나리오(`46b2422`) → 셀. **정직성 한계를 함께 적는다** —
나는 네 `U`를 이미 알고 있었으므로 보장하는 것은 *"U를 모르는 상태에서 정했다"*가 아니라 *"입력 자료와
산정식을 외부 근거·실측에서만 가져왔고 판정에 쓰기 전에 커밋했다"*까지다.
**판정: Part 1 16/16 ADMIT · Part 2 4 ADMIT/4 DENY · 24/24 통과 · 사전 예측 16/16 적중.**
**핵심 관측 — 관측 피크는 예산의 함수가 아니다**: 예산이 297,002,484 B → 2,776,436,561 B로 **9.3배**
달라져도 모델별 HAL peak가 **바이트 단위로 동일**하고(309,416 / 6,208 / 9,382,092 / 131,382,784) 넷 다
그 모델의 `static_per_call_bytes`다(map 분기, E29b). **예산이 정하는 것은 판정이지 실행의 메모리 거동이
아니다.** **전 셀 ADMIT을 본 뒤 예산을 낮추지 않았다**(지시 §9) — 계획 §7이 이 경우의 서술을 측정 전에
정해 뒀다. 판별력은 Part 2(모델 무관 2의 거듭제곱 격자)가 담당하고 **실제 임무 예산이라 부르지 않으며**,
두 part는 별도 파일·별도 판정이고 가드 `e54/7`이 분리를 구조적으로 강제한다.
**기준선 분리 불가**: 사전 고정 규칙 `min(M2) > max(M3)`가 5,156 > 5,324로 거짓 — cFS가 앱 전부를 한
프로세스에 `.so`로 적재해 회차 변동(1,208 kB)이 임무 앱 11개 기여와 맞먹는다. `R_other_apps = 0`은
**측정해서 0이 아니라** `R_OS_cFS`(M2 기반) 안에 이미 있어 이중 계상이 되기 때문이고 그 구분을 레코드에
적었다(D29·D51·D68). **출처 등급**: NASA 1차 출처 4건은 재프로브에서도 전부 차단(curl `000` · WebFetch
`EGRESS_BLOCKED`)이라 **마진 백분율은 어느 취득 바이트에도 없고** `transcribed_from_directive_primary_blocked`
등급이다 — 1차 인용으로 쓰지 않는다. 플랫폼 둘은 실제로 받아 인용문을 바이트에서 대조했다.
**정정**: 계획 산문이 앱 정적 I/O 버퍼를 셋으로 적었으나 **세었더니 여섯**이다(`zeros`·`yv`·중복 `out` 누락) —
인용했으면 바이너리에 있는 항이 오버헤드에서 조용히 빠졌을 것이다(E44).
**방법론 결함(판정 아님)**: wgan 승인 셀 5건의 첫 시도가 `completed 0 < 1`로 실패했는데 **실패한 것은
admission이 아니라 창 길이**다(다섯 셀 전부 ADMIT·MATCH·`mem_init` 기록). E53의 *"10,800초에 4회"*를 나눠
2,700초로 보고 3,000초를 잡았으나 **그 평균에는 부팅·적재를 치르지 않는 2~4회차가 섞여 있다** — E45 교훈의
재발이다. E53 로그의 분해로 5,400초로 고쳐 **5/5 통과**. 실패 원자료는
`cells/undersized_window_first_attempt/`에 사유와 함께 보존했고 **판정 기준·예산·계약·앱 코드는 불변**이다.
**그 시간 수치는 계획 §3.4/작업 규율 4대로 창 산정용이며 성능 증거가 아니다.**
**D93 수정이 독립 발동**: DeepAE 승인 셀들의 `last_run`이 `None`(640원소 출력이 `line[768]` 초과)인데
`last_mem` 폴백이 물어 통과했고, 같은 실행의 b2_resnet 셀은 출력 10원소라 온전해 **잘리는지를 정하는 것이
출력 arity**임이 한 파일 안에서 대조된다(근본 원인은 여전히 열려 있다).
가드 9건 신설, revert 시 3/3 FAIL — **가드 8이 처음에 자기 docstring에 걸려 FAIL**했다(**D85의 뒤집힌
얼굴**: 규칙을 설명하는 문장이 규칙을 *위반*시켰다), docstring·주석을 걷어내고 실행되는 줄만 보도록 고쳤다.
이 컨테이너 **831/831 → 840/840**, 보관 14개 계약 diff 0.
**CI 실측**(커밋 `9493881`, run 290, 3레그 success): `full` **835/835 + 4 SKIP** · `without-iree` **665/665 + 36 SKIP** · `stdlib-only` **665/665 + 36 SKIP**. 세 레그 전부 직전 커밋(E53, run 34690758943: `full` 826/826+4 · 축소 두 레그 656/656+36) 대비 **PASS +9 · SKIP +0**이라 E54 신규 가드 9건이 **전부 나타난다** — 소스 텍스트 검사·보관 JSON 판독뿐이라 툴체인 없이 실제로 돈다. 이 컨테이너(**840/840 + 1 SKIP**)와 `full`의 PASS 차이 **5건**은 E38이 확립한 설명 그대로다(PyYAML 1 · `aarch64-linux-gnu-objdump` 미설치로 정직하게 SKIP되는 2 · 그 툴체인이 없으면 분기가 아예 없어 존재하지 않는 2), SKIP 차이 3건도 같은 셋이다.
**교훈**: ***예산을 계약에서 떼어 내면 비로소 "예산이 정하는 것"과 "모델이 정하는 것"이 갈린다*** ·
***산문이 아니라 코드를 읽는 가드를 써라 — 규칙을 설명하는 문장이 규칙을 만족시킬 수도, 위반시킬 수도 있다.***
**하지 않음(명시)**: 조건부 계층(20/20 `unconditional`) · 의미 동치·정확도 · 재컴파일 · OnAIR 참조 예산 셀 ·
`R_other_apps` 분리 측정 · 지연·전력·WCET.

**v0.58에서 완료된 것 (E55, `docs/EVIDENCE_v0.58_E55.md`)**: **v0.57 필수 후속 작업 P0-1~P0-4.**
연구 책임자 지시(`docs/reviews/MANDATORY_FOLLOWUPS_v0.57_RECOMMENDATION.md`, 원문 보존 sha256 `e7074a20…`)가
선택적 확장이 아니라 **네 항목의 완료**를 요구했고 §2~§5에서 완료 기준을 문장으로 미리 고정했다.
사전 고정 계획 `docs/plans/E55_mandatory_followups.md`(커밋 `a63149b`, **측정 이전**).
**P0-1**: 지시가 준 세 URL이 E54 매니페스트에 **하나도 없는** 경로임을 먼저 확인하고(GSFC-STD-1000은 프로브
기록 자체가 없었다) 세 대상에 **66개 경로**를 재 봐 **NPR 7150.2D §5.4.5 하나를 취득**했다(5개 독립 미러 바이트
합치). **차단의 기전을 이름 붙였다** — 프록시가 돌려준 **101 B** 본문이 `Host not in allowlist`이지 NASA 실패가
아니고, **바이트를 받은 것은 취득이 아니다**. 마진 수치 등급은 `transcribed_from_directive_primary_blocked` **유지**.
**D94**(참조 예산이 저장소 내용으로 재현되지 않음 — glob이 실험마다 자란다): E54 **판정은 불변**이고 무너진 것은
재현성이다. 영향을 단정하지 않고 **측정** — 16셀 전부 **−8,192 B**(보수적 방향) · **판정 뒤집힘 0/16**. 입력을
pin하되 **매번 다시 읽어** 검증하고 드리프트는 형제 파일에 싣는다(**첫 수정이 그 수를 `budgets.json`에 넣어
D94가 자기 수정 위에서 재연됐다**). **P0-2**: 문자 그대로의 처방을 **채택 전에 쟀더니 보관 IR 27/27 전멸**
(결정타 `stream.yield` — MLIR API 99회 vs IR 텍스트 **0회**, 정규식 파서가 원리적으로 볼 수 없다). 그래서
영역을 이름 접두어가 아니라 **자원 흐름**으로 정의하고 지원/비할당확인(사유 기록)/명시적 거절 세 갈래로 분류,
미분류는 **전용 키**로 보고한다(`unresolved`에 접으면 전멸이 재현된다). 네 실물 모델 상한 불변, 보관 14개 계약
diff 0. **D95**(walker가 마지막 chunk 파싱 실패 시 조용히 pre-layout chunk로 폴백; 원인은 IR이 아니라
`iree-compile` **진단 remark 한 줄**) 수정 — 이 발견이 **D87의 기록을 정정한다**(*"`dynamic` 두 파일에
`stream.async.*` 6개"*는 IR의 사실이 아니라 **chunk 선택 artifact**였다). **P0-3**: `max_in_flight_calls = 1`이
E49 축 B의 `ARGUED_FROM_SOURCE`에서 **OBSERVED**로 — 두 C 실행기의 `call_deinitialize` 지점(3+3곳 전수)에
ENTER/EXIT를 두고 `max_active_calls`·`call_counter_balanced`를 기록한다. **E51 stage2의 기록은 그대로 둔다**
(호출부를 읽는 일은 여전히 논증이다). **P0-4 PASS — 네 모델 전부**: 두 셀의 차이가 컴파일 타임 knob 하나뿐이고
예산은 양쪽 다 `P`(컴파일 타임 예산은 `bounded+1` — 더 작게 빌드하면 컴파일러가 admission 실패를 정적으로 접는다).
b2_resnet **309,416** · b3_deepae **6,208** · smartcam **9,382,092** · wgan **131,382,784** — **HAL peak가 넷 다
`P`와 바이트 단위로 동일**하고 `module_ptr_mod64: 0`·`hal_peak_after_append: 0`을 **측정으로** 확인했다
(tightness는 모델의 성질이 아니라 **전제조건의 성질**이다 — E29). 대조 4셀은 같은 예산에서 NOT_ADMITTED·추론 0·
런타임 미생성이라 통과가 조건부 계층에 귀속된다. **단일 모델 의존성 해소**(상수:호출 비 0.006~171.3 양극단) —
**다만 `Bᵤ/P` 이득은 172.30×~1.03×로 모델마다 다르고, 네 셀이 보이는 것은 계층이 작동한다는 것이지 똑같이
유용하다는 것이 아니다.** 여덟 기준 중 **8(기존 출력 기준 일치)은 `null`** — 이 셀들은 SB 텔레메트리를 쓰므로
fixture를 재생하지 않는다(**통과로 적지 않는다**, D29·D51·D68). 사전 보수 둘: `run` 레코드에서 출력 배열 제거
(첫 경로가 OSAL `OS_MAX_FILE_NAME` 20자를 넘어 `-104`로 거부된 것을 **추측하지 않고 사유 필드로** 알아냈다) ·
`WARMUP_CALLS` fail-closed 오버라이드. **방법론**: 대조 셀 기대 키 `init_count: 0`·`e25_mode_active: false`는
**내가 정한 것이지 앱이 말하는 것이 아니었다** — 더 엄격한 `runtime_created: False`로 바꾸고 게스트 재실행 없이
`--reparse`로 오프라인 재판정했다. 12개 배포 트리의 build_info·witness·`.so` 해시 보관(E38/D69 — exe 트리는
저장소 밖이라 사라진다). 이 컨테이너 **840/840 → 860/860 + 1 SKIP**.
**CI 실측**(커밋 `2d3a9ff`, run 308, 3레그 success): `full` **855/855 + 4 SKIP** · `without-iree` **679/679 + 38 SKIP** · `stdlib-only` **679/679 + 38 SKIP**. 직전 기록(E54, run 290: `full` 835/835+4 · 축소 두 레그 665/665+36) 대비 `full`이 **정확히 +20**이라 이 컨테이너의 840→860과 같은 수를 가리킨다. 축소 두 레그는 **PASS +14 · SKIP +2 = 16건**이므로 **20건 중 4건은 거기 나타나지 않는다** — 이름 댈 수 있는 것은 분석 영역 4건(e55/6·6b·7·8)이 구조적 추출기를 요구해 **하나의 정직한 SKIP으로 접히는 것**과 e55b/7이 numpy를 요구하는 것이고, **나머지 구성은 로그의 사유 목록이 집계 문자열이라 이 기록만으로 특정되지 않는다. 추정하지 않고 미상으로 남긴다**(D34, E46 선례). 이 컨테이너(**860/860 + 1 SKIP**)와 `full`의 PASS 차이 **5건**·SKIP 차이 **3건**은 E38이 확립한 설명 그대로다(PyYAML 1 · `aarch64-linux-gnu-objdump` 미설치로 정직하게 SKIP되는 2 · 그 툴체인이 없으면 분기가 아예 없어 존재하지 않는 2).
**교훈**: ***지시가 준 경로가 우리가 재 본 경로와 같은지 먼저 대조하라*** · ***처방을 채택하기 전에 그 처방이
무엇을 죽이는지 재라*** · ***기대 키가 앱이 말하는 것인지 내가 정한 것인지 구분하라.***

**v0.59에서 완료된 것 (E55b, `docs/EVIDENCE_v0.59_E55b.md`)**: **AArch64 cFS copy 적재 경로 — 네 모델 4/4 PASS.**
연구 책임자 지시(`docs/reviews/COPY_MAP_ONAIR_RECOMMENDATION_v0.57.md`, 원문 보존 sha256 `ba7f66d9…`) §3~§4가 공백을
정확히 이름 붙였다 — **E29의 정렬 스윕 64셀은 x86-64 프로브**였고 **AArch64 cFS에서 copy 분기를 실제로 태운 셀은
하나도 없었다**(E26/E26e/E26f의 copy 셀은 전부 x86-64다). 사전 고정 계획 `docs/plans/E55b_copy_path_aarch64.md`
(커밋 `e58216b`, **측정 이전**). 같은 AArch64 vmfb·계약·fixture·예산 `M = Bᵤ`·무조건 승인에서 **정렬 오프셋 8**만
바꿔 `try_map`의 import를 거부시켰다: `mod64 8` · append 후 할당이 **정확히 `C`**(309,440 / 1,063,424 / 8,840,704 /
4,283,648) · 최종 `H`가 **정확히 `P+C`**(618,856 / 1,069,632 / 18,222,796 / 135,666,432) · 출력이 보관 map 셀과
**비트 동일**(340 / 21,760 / 15 / 150,528원소, worst_abs 0.0) · `max_active_calls` 4/4 = 1.
**같은 예산에서 `H`가 `P`와 `P+C`로 갈리는 것이 `Bᵤ = P + C`가 왜 두 분기의 최댓값이어야 하는지의 실행 증거**이고
copy 분기가 그 값에 **정확히 닿는다**. `AI_LEARNER_BLOB_ALIGN_OFFSET`은 **fail-closed 6조건**(비정수·범위 밖·
8의 배수 아님·0 아닌 64의 배수·조건부 빌드)이고 **기본값 0**이라 기존 배포는 바이트 불변이다.
**분기 이름으로 확정하지 않는다**(지시 §3.3) — 앱이 `arm`을 `hal_peak_after_append`에서 유도하므로 라벨은 독립
사실 **옆에** 실린다. **OnAIR은 신규 실험 0건**으로 E33 세 셀을 연결만 하고 **거부 지점**(admission / binding)을
값으로 싣는다(지시 §5).
**적대적 검증이 이 실험 자신의 결함 4건을 찾았고 전부 재현 후 고쳤다 — 네 건 모두 측정값·판정 불변이다.**
**D96**: 사전 등록한 `mem_init` 예측이 *"copy 확정의 네 번째 독립 사실"*이 아니라 **항등식**이었다 — 두 레코드가
같은 세션 할당자의 같은 **최고수위선**(`hal/allocator.h:777`)을 읽고 그 사이의 디바이스 할당은 입력 버퍼 하나·
해제 0이다. 보관 셀 **56개**(5모델)에서 모델당 델타가 단일값이고 **두 분기에서 같아** 분기를 판별하지 못한다.
예측 커밋(`c1aaf32`) 시점에 같은 관계를 만족하는 copy 셀이 **나흘 앞선 커밋**으로 이미 있었다.
***사전 등록된 산술적 귀결은 사전 등록된 관측이 아니다.***
**D97**: *"두 실행의 유일한 차이는 정렬"*이 거짓 — 앱 **바이너리는 고정이 아니었다**(e36b·e37 대비 +195/−11 ·
커밋 4건, e53 대비 +166/−10 · 커밋 2건, 그중 `330c4f3`/D75가 비교 대상 출력을 쓰는 e25 재생 루프 안의
`float yv[]`를 `static`으로). **결과는 약해지지 않는다** — 출력이 그 구간을 가로질러 비트 동일이므로 더 강한
진술이다. **D98**: 통합표의 조건부 **대조** 행이 예산을 하드코딩하고 `allow_conditional_map`을 읽고도 쓰지 않아
**E38/D69가 없애려고 만든 순환**이 돌아와 있었다. **D99**: OnAIR 세 셀에 nanobind 누수 사유를 같은 문장으로
달았으나 두 셀은 그 줄이 **0건**이다(런타임 생성 전 거부) — **D51의 뒤집힌 얼굴**.
**가드** `e55b/12a`(통합표를 게스트 로그에서 다시 유도) · `e55b/14`(출력 기준값 셀의 map 분기를 그 셀의 로그에서
읽는다) · `e55b/15`(앱 소스 델타를 `git diff --numstat`으로 다시 잰다) 신설, `/11`·`/12`·`/13` 수정 — 전부
revert 시 실제 FAIL. **`/12`·`/13`은 커밋된 `summary.json`만 읽어서 원자료를 되돌려도 초록이었다**(D89).
사전 등록 반증 조건 **F2가 거의 공허**함(앱이 `arm`을 `hal_peak_after_append`에서 유도하므로 `copy`가 `C`가 아닌
값 옆에 나올 수 없다 — D72의 형태)도 기계 판독으로 적었다. 이 컨테이너 **860/860 → 866/866 + 1 SKIP**.
...
**하지 않음(명시)**: 반복 호출 장기 수명 · *"실제 배포가 copy를 탄다"*(반대다 — 두 C 실행기는 E29b 이후 자기
blob을 64바이트 정렬로 잡아 **구조적으로 map**이고, copy 셀은 그 정렬을 **일부러 제어**해 만든 것) ·
DeepAE의 TFLite 대비 FAIL 변경(D74 유지) · 프로세스 RSS·시스템 RAM(D78) · AArch64 OnAIR · 지연·성능
(`platform_check.py`가 `FUNCTIONAL_ONLY`) · 조건부 계층(이번 셀은 전부 무조건).
**교훈**: ***사전 등록된 산술적 귀결은 사전 등록된 관측이 아니다*** ·
***"유일하게 바뀐 것"이라고 쓰기 전에 무엇이 고정됐는지를 세어 보라*** ·
***가드가 읽는 것이 문서인지 원자료인지 확인하라.***

**v0.60에서 완료된 것 (E41b, `docs/EVIDENCE_v0.44_E41.md` §10 정오표)**: JAIS 원고 검토를 검증하다
**저장소 자신의 결함 두 건**을 찾아 재현·수정했다. **둘 다 판정·수치 영향 0**이고 바뀌는 것은
*어떤 문장이 관측이었는가*다.
**D100 — 원자료 없는 수치가 판정 문장에 살았다**: *"N=1 HAL 피크는 `local-sync`/`local-task`에서 바이트
동일(b2_resnet 309,416)"*이 네 곳에 있었는데 **보관 셀이 0건**이었다(`results/` **556개 파일 전수 스캔** —
`local-task`가 나오는 8개 파일은 전부 배포 설정 입력이거나 `smartcam_wrong_driver`의 거부 문자열이다).
출처는 계획서 §0 표이고 그 §1은 스스로 *"착수 전 조사 — 실험 아님, 판정 아님"*이라 적는다(D65·D69 계열).
**지우지 않고 쟀다** — `harness/e41_domain_probe.py`가 처음부터 `--driver`를 받고 있었고 local-task 레그만
실행된 적이 없어 **도구 수정 0줄**이었다. 6셀 **6/6 바이트 동일**, 전 셀 `peak == per_call`, 전 셀 결정적
(`results/e41b_driver_peak/`). **주장은 참이었고 근거만 없었다.** **확정하지 않는 것 3가지**를 summary가
기계 판독으로 싣는다 — 적재 분기의 드라이버 간 동일성(프로브가 `copy_buffer`로 적재해 **분기를 고정한 채**
쟀다; 분기를 변화시키는 실험은 E29의 64셀이며 **local-sync 전용**) · 두 C 배포(그 런타임은 local-task가
**컴파일돼 있지 않다**) · N>1.
**D101 — 인용이 한 절 어긋났고 그 문자열이 생성기에 하드코딩돼 있었다**: E52의 전처리 배제 근거가
*"E45 §2.4"*였으나 전처리 항등 표는 **§2.3**에 있다(§2.4는 *사후 특성화(판정 아님)*이고, 같은 문서의 다른
§2.4 인용은 **맞다**). **D91 그 자체이되 더 나쁘다** — `harness/e52_deepae_layers.py:427`에 박혀 있어
`divergence.json`만 고치면 **다음 재생성에서 되살아난다**. 생성기·JSON·이 파일 세 곳을 고치고 EVIDENCE
두 곳에는 규율 5대로 **정오표를 덧붙였다**(사전 등록 계획서와 원문 표는 고쳐쓰지 않는다).
가드 `e41b/1`~`/8` · `e52-ptr/1`~`/5` 신설, **revert 양방향 실측**(생성기만 되돌리면 2건 FAIL, docstring만
되돌리면 2건 FAIL). 이 컨테이너 **866/866 → 881/881 + 1 SKIP**, 보관 14개 계약 diff 0.
**교훈**: ***사전 조사의 수치를 판정 문장으로 옮기는 순간 셀을 남기거나, 옮기지 말아야 한다*** ·
***인용을 고칠 때는 그 문자열을 만들어 내는 자리부터 고쳐라.***
**CI 실측**(커밋 `45aedf7`, run 342, 3레그 success): `full` **875/875 + 5 SKIP** · `without-iree` **699/699 + 39 SKIP** · `stdlib-only` **699/699 + 39 SKIP** — v0.59(run 338) 대비 세 레그 전부 PASS +15 · SKIP +0. **이 기록은 v0.62가 뒤늦게 채웠다**(v0.61 추가로 v0.60이 head에서 내려온 뒤 CI가 D34 가드 1건으로 run 344~362 동안 빨갰다).

**v0.61에서 완료된 것 (E56, `docs/EVIDENCE_v0.61_E56.md`)**: **조건부 계층의 전제 실패 거부를 평가 타깃에서 실행 — 8셀 전부 PASS.**
연구 책임자 지시(**x86-64는 이 논문의 타깃도 검증 수단도 아니다. 모든 실험과 V&V는 AArch64 기반**)에 따라,
그때까지 **x86-64 cFS 두 셀에만** 있던 거부 경로를 AArch64 게스트에서 다시 만들었다. 사전 고정 기준은
`docs/plans/E56_conditional_refusal_aarch64.md`(커밋 `ad2c1ba`, **측정 이전**).
**갭은 실재했다** — 게스트 로그 전수에 `MAP_PRECONDITION` 레코드가 **0건**이었다. 게스트의 조건부 셀(E55/P0-4) 4개는
전부 `module_ptr_mod64: 0`으로 전제가 성립했고, 정렬을 깬 셀(E55b) 4개는 전부 **무조건** 계층이라 거부할 이유가 없었다 —
**두 노브가 독립적으로만 검증돼 있었고 그 조합 셀이 없었다.**
**착수 전 조사가 그 조합을 막는 유형 (B) 과잉 거부를 찾았다**: `AI_LEARNER_BlobAlignOffset()`이 조건부 빌드에서
0이 아닌 모든 offset을 거부했고 사유는 *"이 노브가 게이트를 우회하는 데 쓰여선 안 된다"*였다. **그 사유는 성립하지 않는다** —
조건부 ON + offset이면 `module_ptr_mod64 != 0`이 되고 append 전 검사가 **런타임 생성 전에 거부**한다. 우회 경로가 아니라
**게이트를 발동시키는 유일한 경로**이고, E55b의 문장은 그 실험의 범위 결정이지 안전 성질이 아니었다(D47·D48·D49·D57·D80 계열).
**제거가 아니라 좁히기** — 나머지 여섯 fail-closed 축과 두 방어(append 전/후)는 불변이고, 무조건 빌드는 바이트 불변이다.
**거부 셀 4/4**: `blob_align` state=applied·**mod64 8** → `ADMIT_CONDITIONAL_MAP`(예산 `P`, `budget_source=override`) →
`binding MATCH` → **`MAP_PRECONDITION_UNMET`** → `mem_init` **부재** · 추론 **0** · cFS OPERATIONAL 유지.
**양성 대조 4/4**(같은 바이너리, offset 미설정 — **D97 대비**: 바이너리가 바뀌었으므로 거부가 "게이트 발동"인지
"빌드 파손"인지는 같은 바이너리의 통과 경로로만 가린다): `arm=map` · append 피크 0 · 추론 223/224/139/1 ·
**최종 피크가 `P`와 바이트 단위로 일치**(309,416 / 6,208 / 9,382,092 / 131,382,784).
**x86-64 두 셀을 대체한다** — 디스크에는 남기되 근거로 인용하지 않는다. 대체본이 세 축에서 더 강하다:
**평가 타깃**(개발 호스트 아님) · **실물 4모델**(합성 bigact 1 + DeepAE 1 아님) · **앱 자신의 노브**(shim 아님) ·
4/4가 **런타임 생성 전** 거부(사전 1 + 사후 1 아님).
**D102(부수 발견)**: E55b의 `app_source_delta`가 끝점 둘 중 **하나만 고정**하고 작업 트리와 diff했다 —
`ai_learner.c`가 다시 바뀌는 순간 기록값이 드리프트하고, 가드가 **그 셀들이 빌드된 적 없는 소스에 맞춰
역사적 기록을 고쳐 쓰라고** 요구한다. `cells_built_at`(`3e746a8`)을 싣고 범위로 바꿨다 — **수치 195/11 · 166/10 불변**.
가드가 끝점 미고정을 거부한다. E51 stage2의 줄번호 이동(693→703)도 유도를 다시 돌려 갱신했다.
**방법론**: `e56/2`의 첫 판이 **자기 설명 주석**에 걸려 FAIL했다 — D85(*규칙을 설명하는 문장이 규칙을 만족*)와
E54 가드 8(*규칙을 설명하는 문장이 규칙을 위반*)의 **세 번째 얼굴**이다. 주석을 걷어내고 실행되는 줄만 보게 고쳤다.
가드 16건, 이 컨테이너 **897/897 + 1 SKIP**, revert 시 `e56/2` 실제 FAIL.
**주장하지 않음**: 이 실험을 위해 x86-64에서 아무것도 돌리지 않았다 · *"런타임 생성 전 거부"*는 `mem_init` **부재**로
추론한 것이다(D80, 직접 신호는 여전히 없다) · 양성 대조는 SB 텔레메트리를 쓰므로 **출력 동치를 주장하지 않는다** ·
정확도·지연·전력은 범위 밖 · 합성 모델의 AArch64 이식은 하지 않았다.
**CI 실측**(커밋 `0e1b9ac`, run 354, 3레그 **failure**): `full` **890/891 + 5 SKIP** · `without-iree` **714/715 + 39 SKIP** · `stdlib-only` **714/715 + 39 SKIP** — 세 레그의 FAIL 1건은 같은 D34 가드(v0.60 CI 기록 부재)이고 E56 판정과 무관하다. 검사 수 +16 · SKIP +0이라 E56 가드 16건이 전부 나타난다.

**v0.62에서 완료된 것 (E59, `docs/EVIDENCE_v0.62_E59.md`)**: **정보 수준 비교를 평가 타깃(AArch64) 아티팩트로** — 원고 §VI.A의 근거였던 E35는 4구성 중 3개가, E27의 드리프트 아티팩트는 전부가 x86-64였다. 사전 고정 기준 `docs/plans/E59_info_levels_aarch64.md`(커밋 `7acc660`, 측정 이전). **Part A**: 원고 표의 네 AArch64 계약에서 굳힌 아티팩트 전용 분석기가 세 수치 **4/4**·커널 스택 **4/4** 일치(24 정책 셀은 따름정리, D72). **Part B**: 같은 네 MLIR을 IREE **3.10.0rc20260107**(E27과 같은 드리프트 리비전)로 AArch64 재컴파일, 같은 명령의 3.11 대조가 보관 수치를 **4/4 재현**. (c) layout 기반 **4/4 수치 + note** · (b) 출하한 아티팩트 전용 **4/4 불가능 값**(per_call = 입력 텐서 하나 < I+O, `unresolved=[]`) · (b′) 굳힌 변종 **4/4 `C4_DISASM` 거부**. E27의 n=1(합성·x86-64)이 **네 실물 모델·AArch64에서 n=4**가 됐다 — 리비전은 여전히 하나다. 수집기 첫 판이 (b′) 거부 사유를 없는 키에서 읽은 결함(D51의 작은 판본)은 판정 전에 고쳤다. **CI 기록 결함도 이 버전이 닫는다** — v0.60·v0.61 기록을 뒤늦게 채웠다. 가드 `e59/1`~`/8`, 이 컨테이너 **897/897 → 905/905 + 1 SKIP**.
**CI 실측**(커밋 `623df3b`, run 370, 3레그 success): `full` **898/898 + 6 SKIP** · `without-iree` **721/721 + 41 SKIP** · `stdlib-only` **721/721 + 41 SKIP**. v0.61(run 354) 대비 세 레그 전부 **검사 수 +8**이라 `e59` 8건이 전부 나타난다(`full` PASS +8·SKIP +1 — D34 가드가 FAIL에서 PASS로 돌아왔고 `e59/7`이 objdump 부재로 SKIP; 축소 두 레그 PASS +7·SKIP +2, 그 둘의 구성은 사유 목록이 집계 문자열이라 특정하지 않는다).

**v0.63에서 완료된 것 (E60, `docs/EVIDENCE_v0.63_E60.md`)**: **DeepAE 수치 불일치의 층별 위치를 평가 타깃(AArch64 게스트)에서** — 원고 §V.G의 층 분해 근거였던 E52는 x86-64였다. 사전 고정 기준 `docs/plans/E60_deepae_layers_aarch64.md`(커밋 `623df3b`, 셀 실행 이전). 배치 AArch64 MLIR을 층 경계마다 자른 진단 모듈 10개를 게스트의 IREE 휠로 실패 창 하나에 실행했다. **V 성립** — 체인의 마지막 모듈 출력이 E48이 보관한 AArch64 **C 런타임** 출력과 640원소 **비트 동일**(서로 다른 빌드). **L1 = 6 · L2 = 9**(마지막 층 46원소 = E48의 수), 층별 위반 `[0,0,0,0,0,0,1,1,0,46]`. **L3: 순차 float32 누산과 비트 동일한 층 0/10** — 첫 층부터 다르고, 마지막 층은 기준값·순차 f32·float64 **세 축 전부와** 허용오차 밖(46·30·19). **E52(x86-64)와 위치가 같다**(L1 6, 대량 9). **기전은 주장하지 않는다** — 디스어셈블하지 않았고, AArch64는 FMA가 기본 ISA라 E52의 FMA 대조가 성립하지 않는다. 쓸 수 있는 것은 *"평가 타깃의 출력은 어느 층에서도 순차 누산과 비트 동일하지 않고, 허용오차 초과는 층 6에서 처음 나타나 마지막 층에서 대량이 된다"*까지다. 게스트 층 출력(모델 출력이지 입력이 아님)을 보관하고 가드 `e60` 8건이 V를 **그 원자료에서 다시 유도**한다(D89); 층 비교 라이브 재실행은 E52와 같이 `E52_INPUTS`로 게이트. revert 시 2건 실제 FAIL. 이 컨테이너 **912/912 + 2 SKIP**(`E52_INPUTS`를 주면 914/914).
**CI 실측**(커밋 `a022dd2`, run 372, 3레그 success): `full` **905/905 + 7 SKIP** · `without-iree` **727/727 + 43 SKIP** · `stdlib-only` **727/727 + 43 SKIP** — v0.62(run 370) 대비 세 레그 전부 검사 수 +8(`full` PASS +7·SKIP +1, 축소 두 레그 PASS +6·SKIP +2 — 층 비교 라이브 재실행은 실입력이, V 재유도는 numpy가 없으면 SKIP).

**v0.64에서 완료된 것 (E58, `docs/EVIDENCE_v0.64_E58.md`)**: **적재 분기 결정 요인을 평가 타깃의 비행 앱에서 8 정렬 클래스 전부로** — 원고 §V.C·표 `tab:arms`의 근거였던 E29(64셀 스윕·7모델 쌍 적재)는 x86-64 개발 호스트였다. 사전 고정 기준 `docs/plans/E58_alignment_sweep_aarch64.md`(커밋 `4d10203`, 측정 이전). 4모델 × 오프셋 {0,8,…,56} = **32셀 전부 유효 · map 4 · copy 28 · 제3의 값 0** — map은 **오프셋 0에서만**, 나머지 일곱에서 append 할당이 **정확히 `C`**(309,440 / 1,063,424 / 8,840,704 / 4,283,648). **한 모델의 8셀은 한 아티팩트·한 바이너리**라 셀 사이의 유일한 설정 차이가 앱 자신의 정렬 노브다(E55b/D97의 단서가 이 스윕 안에는 없다). 부수(판정 아님): 같은 DeepAE 빌드의 최종 피크가 오프셋 0에서 6,208, 8에서 1,069,632 — **172.30배를 한 빌드 안에서**. 기전(`iree_hal_heap_buffer_wrap()`의 정렬 조건)은 여전히 **소스 읽기**이고 이 실험은 그 귀결을 타깃에서 관측할 뿐이다. 가드 `e58` 6건(raw log 재유도, revert 시 FAIL), 이 컨테이너 **918/918 + 2 SKIP**. x86-64 E29 셀은 디스크에 남기되 원고 근거로 인용하지 않는다.
**CI 실측**(커밋 `7c4ff51`, run 382, 3레그 success): `full` **911/911 + 7 SKIP** · `without-iree` **733/733 + 43 SKIP** · `stdlib-only` **733/733 + 43 SKIP** — v0.63(run 372) 대비 세 레그 전부 PASS +6 · SKIP +0이라 `e58` 6건이 전부 나타난다.

**v0.65에서 완료된 것 (E61, `docs/EVIDENCE_v0.65_E61.md`)**: **원고 표 `tab:outputs`의 기준값을 평가 타깃에서 생성했다 — DeepAE 판정 수치가 바뀌었다.** 사전 고정 기준 `docs/plans/E61_reference_outputs_on_target.md`(커밋 `964b18d`, 게스트 실행 이전). 피검 쪽(E48·E53·E60)은 전부 AArch64였으나 **기준값 쪽은 전부 지상 측 x86-64 LiteRT**였다(**D104**). 같은 LiteRT 릴리스(2.2.0)의 aarch64 휠로 게스트에서 다시 만들자 **기준값 자체가 네 모델 전 샘플에서 비트가 달랐다**(최대 DeepAE 1.045e-03). 재판정(E25 규칙 무변경): ResNet **PASS 0/2,000** · SmartCam **PASS 0/57** · WGAN **PASS 0/150,528** · **DeepAE FAIL 139/21,760, 4창**(지상 측 기준값에서는 46, 1창) — **피검 출력은 한 비트도 바뀌지 않았고 움직인 것은 기준값**이다. E60 창을 게스트 LiteRT의 보존 실행 중간값과 대조하면 층별 위반 `[0,…,0,37]` — **마지막 층 전의 어느 층도 초과하지 않는다**(E60의 *"층 6에서 처음"*은 기준값 쪽 교차-ISA 차이였다). 계획대로 **원고는 E61 수치를 쓴다**. 가드 `e61` 7건(게스트 레코드 재유도, revert 시 FAIL), 이 컨테이너 **925/925 + 2 SKIP**. **교훈**: ***"타깃에서 검증했다"는 피검 쪽만이 아니라 기준값 쪽에도 걸린다.***
**CI 실측**(커밋 `19e8f43`, run 386, 3레그 success): `full` **918/918 + 7 SKIP** · `without-iree` **739/739 + 44 SKIP** · `stdlib-only` **739/739 + 44 SKIP** — v0.64(run 382) 대비 `full` PASS +7 · SKIP +0, 축소 두 레그 PASS +6 · SKIP +1(`e61/7` 재유도가 numpy 부재로 SKIP)이라 `e61` 7건이 전부 나타난다.

**v0.66에서 완료된 것 (E57·E57b·E57c, `docs/EVIDENCE_v0.66_E57.md`)**: **OnAIR 플러그인 경로를 평가 타깃(AArch64 게스트)에서 닫았다.** 사전 고정 기준 셋(`d82f866`·`da5b263`·`dff9a29`, 전부 셀 실행 이전); 원안의 x86-64 개발 호스트 실행 환경은 채택하지 않았다. **A: 판정 동치 8/8** — 네 모델 × {`B_u−1`, `B_u`}에서 OnAIR(`admission_policy.decide`)와 비행 앱(C 매크로)이 전부 같다. **서로 다른 두 구현의 대조**라 D72의 따름정리가 아니고, `B_u−1`은 `runtime_created=false`를 **직접 레코드**로 남긴다(D80의 부재 추론이 필요 없다). **B: F4 발동(계획이 미리 정한 "결과")** — append 피크 0, 호출 1 피크 `P`, **호출마다 live +`O`**(40/2,560/12/602,112), 피크 `P+(n−1)O` → **전제 A(호출 사이 출력 해제)가 이 경로에서 성립하지 않는다**(**D103**; D60의 *"해제 미검증"*이 관측이 됐다). **E57b**: 보유는 **호스트 readback 경로** — 결과를 버리면 live 0, `gc.collect()`로 풀리지 않는다. **E57c**: DeepAE 450회에서 **417번째 호출에 승인 예산 초과**(예측 417) — F5 관측. 판정은 두 통합 지점 사이에서 옮겨지지만 **메모리 전제는 cFS에서 성립하고(E55) 여기서는 깨진다**. 보조(판정 아님): OnAIR 출력이 보관 cFS 출력과 대조 가능한 651개 추론 전부 비트 동일. 플러그인은 고치지 않았다(수정은 별도 사전 등록). 가드 `e57` 7건(재유도, revert 시 FAIL). 이 컨테이너 **932/932 + 2 SKIP**(`E52_INPUTS` 미설정 시).

**CI 실측**(커밋 `f60de08`, run 388, 3레그 success): `full` **925/925 + 7 SKIP** · `without-iree` **745/745 + 45 SKIP** · `stdlib-only` **745/745 + 45 SKIP** — v0.65(run 386) 대비 `full` PASS +7 · SKIP +0, 축소 두 레그 PASS +6 · SKIP +1(`e57/7` 재유도가 numpy 부재로 SKIP)이라 `e57` 7건이 전부 나타난다.

**v0.67에서 완료된 것 (E63, `docs/EVIDENCE_v0.67_E63.md`)**: **원고 표 7을 모델마다 한 앱 빌드로 닫고, 조건부 정책의 하한 `M = P − 1`을 실행했다** — v19 원고 메타리뷰 A의 M2(*표 7의 map·copy 열이 다른 빌드*)와 B의 §4(*조건부 정책의 `P − 1` 미실행*). 사전 고정 기준 `docs/plans/E63_single_build_peaks_and_conditional_floor.md`(커밋 `78dfda1`, 셀 실행 이전). **재컴파일 0 · 새 빌드 0.** 착수 전 조사가 빈칸을 **한 칸**으로 좁혔다 — E58 오프셋 0 셀이 같은 E55b 빌드에서 세 모델의 map 최종 피크를 이미 남겼고 WGAN만 90초 창이라 없었다. **Q1 PASS**: E55b 무조건 빌드(`e55b_wgan_copy`)에서 WGAN 오프셋 0 → `ADMIT`·`MATCH`·`module_ptr_mod64` 0·append 0(map)·추론 2·최종 피크 **정확히 `P` = 131,382,784**, 같은 fixture를 재생한 E55b copy 셀과 출력 비트 동일(부수). 표 7의 두 열이 **모델마다 한 바이너리**다 (게스트 sha256 = 보관 4/4, 두 시나리오 파일이 같은 경로) — **숫자는 그대로이고 바뀌는 것은 교란 서술**이다. **Q2 PASS 4/4**: E56 조건부 빌드(`allow_conditional_map=1`, 판정 전 기록)에 `P − 1` → `NOT_ADMITTED`·`mem_init`/`map_branch`/`run`/`mem` 0·cFS 운영 유지. *"런타임 생성 전"*은 여전히 레코드 **부재**로 추론한다(D80). 가드 `e63` 6건(`e63/6`은 raw log 재유도, revert 시 실제 FAIL). 이 컨테이너 **938/938 + 2 SKIP**.

**CI 실측**(커밋 `c52bee3`, run 396, 3레그 success): `full` **931/931 + 7 SKIP** · `without-iree` **751/751 + 45 SKIP** · `stdlib-only` **751/751 + 45 SKIP** — v0.66(run 388) 대비 세 레그 전부 PASS +6 · SKIP +0이라 `e63` 6건이 전부 나타난다.

**v0.68에서 완료된 것 (E62, `docs/EVIDENCE_v0.68_E62.md`)**: **OnAIR 플러그인 경로의 출력 버퍼 보유(D103)를 기전까지 재고, 고치고, 강제했다 — PASS(P1~P9, 반증 발동 0).** 사전 고정 기준 `docs/plans/E62_onair_output_release.md`(커밋 `046a7af`, 셀 실행 이전). v19 원고 메타리뷰 A M1(*강제 단계*)·B R1(*판정 이전 ≠ 전제 충족*). **기전**: `asarray` 뒤 매핑 객체 참조가 **+1** 남고 모든 참조를 놓아도 live **`O`** — 버퍼 프로토콜 읽기는 0·0·같은 출력. **수정**(`output_readback = buffer_protocol`, 새 기본값): 네 모델 + **DeepAE 450회 전부 모든 호출 피크 = `P`**, 승인 예산 초과 0(E57c가 417번째에 넘은 그 텔레메트리), 출력 E57과 호출별 비트 동일. **대조**(`asarray`, 같은 빌드)가 E57 시계열을 정확히 재현 — 노브 하나 귀속. **강제**(`enforce_output_release`, 기본 off): 대조 3모델을 첫 호출 직후 정지, 수정 3모델 위반 0. WGAN 강제 셀은 미실행. D103은 철회하지 않고 조치 칸에 E62를 적었다. **교훈**: ***판정은 옮겨지지만 전제는 경로마다 유지하고 검사해야 한다 — 같은 명세가 두 통합 지점에서 다른 운명을 맞은 이유를 고치는 것은 명세가 아니라 그 경로의 readback이었다.*** 가드 `e62` 11건, 이 컨테이너 **949/949 + 2 SKIP**.

**CI 실측**(커밋 `e52c934`, run 398, 3레그 success): `full` **942/942 + 7 SKIP** · `without-iree` **762/762 + 45 SKIP** · `stdlib-only` **762/762 + 45 SKIP** — v0.67(run 396) 대비 세 레그 전부 PASS +11 · SKIP +0이라 `e62` 11건이 전부 나타난다.

**v0.69에서 정정된 것 (D105 · D106 · D107, `docs/EVIDENCE_v0.69_D105_D106.md`)**: 20차 원고 메타리뷰에 답하려고 원고 문장의 근거를 병렬 조사하다 찾았다. **보고된 수치·판정은 불변.** **D105**: 구조적 walker가 **호출된 함수를 따라가지 않고** 할당을 **나타난 횟수만큼** 센다 — 수작업 편집 IR에서 callee 할당을 빼고 618,856을, 루프 안 할당을 한 번만 세어 622,952를 발행했다. 보관 30개 엔트리에 해당 구조가 없어 평가 불변. 이제 `scf.*`·`cf.*`·`affine.*`·`func.call`·`util.call`이 엔트리에 있으면 **오버라이드 없이 거부**(과잉 거부 0/30). **D106**: E51의 *"비상수 크기면 헤더가 생성되지 않는다"*는 **틀렸다** — 헤더 생성기를 잘못 불러(rc 2) 양성 대조군까지 실패한 것을 관측으로 적었다. 바른 사실은 **헤더가 `CONTRACT_BOUND_KNOWN 0`으로 생성되고 비행 앱이 Init에서 `UNKNOWN_BOUND`로 거부**한다(E14 A8 게스트 관측)는 것이다. **D107**(기록만): `validity.compiler`는 생성 환경의 컴파일러다. 가드 `d105`·`d106` 10건, `e55/6` 조용한 건너뛰기 제거. 이 컨테이너 **959/959 + 2 SKIP**. **교훈**: ***"분류할 수 없으면 발행하지 않는다"가 자원을 나르는 op에만 걸려 있었다 — 할당 횟수를 바꾸는 구조는 다른 축이다*** · ***양성 대조군은 판정에 쓰는 모든 단계를 지나야 한다.***

**CI 실측**(커밋 `1c5b811`, run 402, 3레그 success): `full` **952/952 + 7 SKIP** · `without-iree` **770/770 + 47 SKIP** · `stdlib-only` **770/770 + 47 SKIP** — v0.68(run 398) 대비 `full` PASS +10 · SKIP +0이라 `d105`·`d106` 10건이 전부 돌고, 축소 두 레그는 PASS +8 · SKIP +2다.

**v0.70에서 완료된 것 (E64, `docs/EVIDENCE_v0.70_E64.md`)**: 원고 v25 사실 검증이 찾은 **범위 밖 근거**를 평가 타깃 근거로 교체했다.
사전 고정 기준 `docs/plans/E64_aarch64_evidence_for_manuscript.md`(커밋 `6578481`, 측정 이전). **A PASS**: 보관 ResNet **AArch64** layout
표현 편집 6셀 중 배치 가능 헤더 0, 양성 대조 618,856 — 그리고 원고의 분류 서술을 바로잡는다: **두 이름 계열(`stream.resource`·`stream.tensor`)
안의 미인식 op는 전용 키가 아니라 `unresolved` → 상한 없는 명세**(`BOUND_KNOWN 0`, 비행 앱 Init 거부)이고, 전용 키로 생성을 멈추는 것은
계열 **밖**의 자원 운반 op다. 파싱 불가 op는 walker가 이전 print로 물러나 불일치로 멈춘다(E51의 x86-64 셀이 보인 경로). **B PASS**: AArch64
표현 11개 미분류 0 — **layout 표현에는 대상 표기가 없고 11개 중 8개가 x86-64 컴파일본과 바이트 동일**하므로 대상은 생성 호출의 디렉터리로
판별한다; 평가 명세 4개 재생성 세 수치 불변·헤더 바이트 동일. **C PASS**: 컴파일러 통계를 명세와 **같은 호출**에서 — 상수 = C, transient = T + O 4/4,
동적 모델 JSON·CSV 0. **D**: 게스트에서 순차 단정도·배정도 참조 재계산(30·19·비트 동일 0/10, 호스트 값과 동일), DeepAE 이상 점수 순위 동일
(561쌍 역전 0, 최대 상대 차 1.02×10⁻⁵ — AUC는 기록만, 정확도 아님), 게스트 환경 첫 원자료 기록(이번 부팅: 1 vCPU·974,848 kB·Ubuntu 24.04.4·
LiteRT 2.2.0·IREE 3.11.0·numpy 2.3.5). 가드 `e64` 7건(원자료 재유도·라이브 재실행, revert 시 FAIL). 이 컨테이너 **959/959 + 2 SKIP → 966/966 + 2 SKIP**.
**교훈**: ***표현 파일이 대상에 무관하다면 "어느 대상의 표현인가"는 파일이 아니라 그것을 만든 호출이 정한다.***

**CI 실측**(커밋 `3147684`, run 404, 3레그 success — `ef09a14`에는 독립 run이 없고 그 후속 커밋의 run이 E64를 포함한다): `full` **959/959 + 7 SKIP** · `without-iree` **776/776 + 48 SKIP** · `stdlib-only` **776/776 + 48 SKIP** — v0.69(run 402) 대비 `full`은 **PASS +7 · SKIP +0**이라 `e64` 7건이 전부 돌고, 축소 두 레그는 **PASS +6 · SKIP +1 = 7**이다(SKIP 하나의 구성은 사유 목록이 집계 문자열이라 특정하지 않는다).

**v0.71에서 정정된 것 (D108, `docs/EVIDENCE_v0.71_D108.md`)**: 분석기가 **모듈 초기화 영역의 제어 흐름을 검사하지 않았다** — D105는
엔트리만 거부했고, 두 추출기가 상수 할당을 나타난 횟수만큼 세므로 초기화 영역의 루프·분기는 둘을 똑같이 속인다(편집 IR에서 수정 전
618,856 발행). 이제 `did_map`을 조건으로 하는 `scf.if`와 그 `scf.yield`만 허용하고 나머지는 오버라이드 없이 거부한다. 보관 30개 과잉 거부 0,
**수치 영향 없음**. 원고 L213의 *"분석기가 map 시도와 copy 대안을 해석한다"*는 틀렸다 — C는 packed composite 크기를 한 번 읽은 것이다. 가드 `d108` 4건(revert 시 FAIL), 이 컨테이너 **966/966 + 2 SKIP → 970/970 + 2 SKIP**.
...
- *"OnAIR 경로에서 출력 버퍼 해제를 검증했다"* → **D60으로 철회**. 원자료가 종료 시점 미해제 인스턴스를 보고한다. 반대로 *"누수가 있다"*도 쓸 수 없다 — 상태는 **미검증**이다 (**v0.68/E62 이후**: 이전 readback(`asarray`)의 보유는 기전까지 측정됐고, 버퍼 프로토콜 readback에서는 네 모델·450회 전부 피크 = `P`다 — 쓸 수 있는 것은 *"이 휠·이 드라이버에서 readback을 바꾸자 출력 해제 전제가 회복됐고, 강제 검사가 이전 readback을 첫 호출 뒤 멈췄다"*까지이고 상류 결함 확정·수정이라고는 쓰지 않는다. **v0.66/E57 이후**: AArch64 게스트의 IREE 파이썬 휠 경로에서는 **호출마다 출력 버퍼 하나가 남는 것을 관측**했다 — D103. 쓸 수 있는 것은 *"이 휠·이 드라이버의 OnAIR 플러그인 경로에서 호스트 readback이 출력 버퍼를 호출마다 보유하고, DeepAE는 417회째에 승인 예산을 넘었다"*까지이고, IREE 상류 결함이라고는 쓰지 않는다)
...
- *"분류하지 못한 자원 op은 전부 전용 키로 명세 생성을 멈춘다"* → **아니다**(E64). 두 이름 계열 **안**의 미인식 op는 `unresolved`가 되어
  **상한 없는 명세**가 나오고(헤더 `BOUND_KNOWN 0`, 비행 앱 Init에서 거부), 전용 키로 멈추는 것은 계열 **밖**의 자원 운반 op다. 분류는 **엔트리에만** 적용된다
- *"분석기가 호출·루프·분기를 처리한다 / 그런 엔트리의 상한을 낸다"* → **아니다**(D105, v0.69). 분석기는 호출된 함수를 따라가지 않고 할당 op을 **나타난 횟수만큼** 센다. v0.69부터 엔트리에 `scf.*`·`cf.*`·`affine.*`·`func.call`·`util.call`이 있으면 **계약을 발행하지 않는다**(오버라이드 없음). 평가한 네 엔트리에는 그런 op이 없고 region을 가진 op은 `stream.cmd.execute`·`stream.cmd.concurrent`뿐이다. 쓸 수 있는 것은 *"지원하지 않는 구조는 거부한다"*까지다
...
- *"DeepAE FAIL의 원인은 IREE가 틀렸기 때문이다 / LiteRT가 틀렸기 때문이다"* → **E52가 금지했다.**
  원인은 **누산 순서**이고 세 구현이 세 순서를 쓴다(x86-64 IREE는 float32 **순차** 누산과 비트 동일,
  LiteRT와 AArch64 IREE는 각각 다르다). **셋 다 float32 한계 안이고 제3의 기준값은 없다** — float64와
  순차 float32는 **보조 축**이지 다수결의 표가 아니다. 쓸 수 있는 것은 *"최초 불일치는 L6이고, 누적
  오차(51.9배)가 유효 문턱 상승(2.46배)을 앞질러 L9에서 대량 실패한다"*까지다
- *"E52가 컴파일러의 생성 코드를 확인했다"* → **디스어셈블하지 않았다**. 관측은 *"출력이 순차 참조와
  비트 동일"*까지이고, *"스칼라 루프를 생성했다"*는 쓸 수 없다
- *"DeepAE를 AArch64에서 레이어별로 쟀다"* → **아니다**. AArch64 칸은 E48이 남긴 **보관 원자료 판독**
  (최종 출력 한 벌)이고, 레이어 분해는 x86-64에서만 했다
- *"참조 기반 예산에서 네 모델이 전부 승인됐으므로 이 방법이 실용적이다"* → **E54가 보인 것은 그 반대 방향의
  사실이다**. 전 셀 ADMIT의 직접적 이유는 산정이 느슨해서가 아니라 **이 모델들이 이 급의 플랫폼에 비해 작기
  때문**이다(가장 빡빡한 `PB_pdr50`에서도 최대 모델 WGAN이 2.19배 여유). 쓸 수 있는 것은 계획 §7이 측정 전에
  정한 문장까지다 — *"선정한 참조 플랫폼과 자원 할당 조건에서는 네 모델이 모두 배정 예산 안에 있었다."*
- *"Part 2의 2의 거듭제곱 격자가 실제 임무 예산이다"* → **아니다**(지시 §9). 격자가 모델과 무관하게 고정돼
  계약값의 함수가 아니라는 점에서 `U−1`보다 낫지만 임무가 배정한 값은 아니며, Part 1과 합쳐 하나의 배치
  사례로 쓰지 않는다. 가드 `e54/7`이 한 파일에 두 part가 섞이는 것을 구조적으로 막는다
- *"E54가 예산이 물리 RAM을 예약함을 보였다"* → **아니다**. `R_reserved = 0`이고 그 이유는 임무 선언이 없고
  E44가 소스 85파일에서 예약 가능 호출을 **0건**으로 셌기 때문이다. 예산은 앱에 **부여한 값**이다
- *"`B_contract`와 `U`를 비교했으므로 RSS와 HAL peak가 같은 것이다"* → **아니다**(D78). 비교가 건전한 근거는
  이 배포의 드라이버가 `local-sync`라 **HAL 바이트가 프로세스 RSS의 부분집합**이라는 것뿐이고, 그것은
  충분조건 시험이지 `HAL peak == RSS`의 허가가 아니다
- *"`R_other_apps`를 0으로 측정했다"* → **아니다**(E54 §3.1). 사전 고정 규칙 `min(M2) > max(M3)`가 거짓이라
  **분리되지 않았고**, 0인 이유는 `R_OS_cFS`(M2 기반)가 이미 그 앱들을 포함해 다시 더하면 이중 계상이기
  때문이다. *"측정한 결과 0"*과 *"다른 항 안에 이미 들어 있음"*은 다른 진술이다
- *"NASA 생명주기 마진을 1차 출처에서 인용했다"* → **쓸 수 없다**(E54 §2.1). 네 출처 전부 차단됐고 마진
  백분율은 **이 세션이 받은 어떤 바이트에도 없다** — 등급은 `transcribed_from_directive_primary_blocked`다
- *"WGAN 승인 셀의 첫 실패가 admission 결함이었다"* → **아니다**(E54 §9.1). 다섯 셀 전부 `ADMIT`·`MATCH`·
  **`mem_init` 기록**이었고 실패한 것은 **테스트 창 길이**다. 그 진단에 쓴 시간 수치는 계획 §3.4/작업 규율 4대로
  **창 산정용이며 성능 증거가 아니다** — 이 게스트는 `platform_check.py`가 `FUNCTIONAL_ONLY`를 반환한다
- *"조건부 상한이 네 모델에서 검증됐으므로 조건부 계층이 일반적으로 유용하다"* → **E55/P0-4가 보인 것은
  계층이 네 모델에서 *작동한다*는 것까지다.** `Bᵤ/P` 이득은 DeepAE **172.30×** · ResNet 2.00× ·
  SmartCam 1.94× · **WGAN 1.03×**로 상수 비중의 함수이고, 유용성은 모델마다 다르다(E46이 정량화)
- *"P0-4 셀이 기존 AArch64 출력 기준을 만족했다"* → **아니다. 그 기준은 `null`이다**(여덟 기준 중 8번).
  그 셀들은 Software Bus 텔레메트리 바이트를 feature로 쓰므로 E31/E48 fixture를 재생하지 않는다 —
  미평가이지 통과가 아니다(D29·D51·D68). 그 비교는 E55b의 copy 셀이 같은 fixture를 재생해 수행한다
- *"NASA 마진 수치가 1차 출처에서 확인됐다"* → **여전히 아니다**(E55/P0-1). 66개 경로를 재서 취득한 것은
  **NPR 7150.2D §5.4.5 하나**이고, 마진 백분율은 어느 취득 바이트에도 없어 등급이
  `transcribed_from_directive_primary_blocked` 그대로다. 취득 실패를 *"조달 불가"*라고도 쓰지 않는다 —
  기전은 **이 컨테이너 프록시의 호스트 allowlist 거부**(101 B 본문 `Host not in allowlist`)이지 NASA 실패가 아니다
- *"`max_in_flight_calls = 1`이 이제 모든 경로에서 관측됐다"* → **두 C 실행기에 한해서다**(E55/P0-3).
  OnAIR Python 경로에는 그 계측이 없고, E51 stage2가 적은 *"호출부를 읽는 것은 논증이다"*도 그대로다
- *"구조적 walker가 이제 모든 resource op을 분류한다"* → **자원 흐름으로 정의한 영역 안에서다**(E55/P0-2).
  `!stream.resource`를 만지지 않는 op은 애초에 영역 밖이고, 미분류는 **전용 키**로 보고돼 계약 생성이 거부된다.
  *"화이트리스트 밖은 전부 unresolved"*는 **채택 전에 재서 27/27 전멸로 기각한 처방**이다

- *"copy 셀의 `mem_init` 예측이 맞았으므로 copy 분기가 확정된다"* → **아니다. 항등식이다**(D96/E55b).
  `mem_init.hal_peak`와 `hal_peak_after_append`는 같은 세션 할당자의 같은 **최고수위선** 필드를 두 시점에
  읽고 그 사이의 디바이스 할당은 입력 버퍼 하나·해제 0이다. 델타는 **두 분기에서 같아** 분기를 판별하지
  못한다(보관 56셀 실측). 그것이 증언하는 것은 **계측의 자기정합성**이고, copy 확정의 근거는
  `hal_peak_after_append == C`와 보관 map 셀 대비 `H = P` vs `P+C`다
- *"copy 셀과 보관 map 셀의 유일한 차이는 모듈 이미지의 정렬이다"* → **거짓**(D97/E55b). 앱 **바이너리는
  고정이 아니다** — e36b·e37 대비 +195/−11(커밋 4건) · e53 대비 +166/−10(커밋 2건)이고 그중 `330c4f3`(D75)은
  비교 대상 출력을 쓰는 e25 재생 루프 안의 `float yv[]`를 `static`으로 바꿨다. 쓸 수 있는 것은
  *"유일하게 바뀐 **설정**은 정렬이고, vmfb·계약·fixture 바이트·예산·승인 모드가 고정이다"*까지다
- *"사전 등록 반증 조건 F2가 한 번도 발동하지 않았으므로 copy 분기가 맞다"* → **F2는 거의 공허하다**
  (E55b §8, D72의 형태). `ai_learner.c:614-615`가 `arm`을 `hal_peak_after_append`에서 유도하므로 앱은
  `copy`를 `CONTRACT_CONST_BYTES`가 아닌 값 옆에 **쓸 수 없다**
- *"E55b가 AArch64 cFS에서 OnAIR을 실행했다 / OnAIR 메모리를 쟀다"* → **아니다**(지시 §5). OnAIR은
  **신규 실험 0건**이고 E33의 x86-64 셀을 **연결**만 했다. HAL peak는 그 경로에 계측이 없어 사유를 붙인
  `null`이고, 출력 객체 해제는 D60 이후 **미검증**이다 — 그 사유는 **셀마다 다르다**(D99: `p_deny`·
  `p_mismatch`는 런타임 생성 전 거부라 nanobind 줄이 0건이고, *"미검증"*이 아니라 *"해당 없음"*이다)
- *"실제 배포가 copy 분기를 탄다 / copy가 이 저장소의 기본 거동이다"* → **반대다**(E55b §7). 두 C 실행기는
  E29b 이후 자기 blob을 64바이트 정렬로 잡으므로 **구조적으로 map**이고, copy 셀은 그 정렬을 **일부러
  제어**해 만든 것이다. 관측된 배포 거동의 보고가 아니다

- *"DeepAE는 실입력에서 46원소·1창이 기준을 넘는다"* / *"최초 초과는 층 6이다"* → **E61로 대체**. 그 수치는 **지상 측
  x86-64 LiteRT가 만든 기준값**에 대한 것이었다(D104). 평가 타깃의 LiteRT가 만든 기준값으로는 **139원소·4창**이고 E60 창의
  층별 초과는 **마지막 층뿐**이다. 기준값 구현도 ISA마다 다른 값을 내므로(네 모델 전 샘플) *"어느 기준값이 옳다"*고는 쓰지 않는다
