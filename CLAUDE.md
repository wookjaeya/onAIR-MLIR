...
`.tflite`와 바이트 동일**) · **전처리**(E45 §2.4) · **FMA**(`-fma`를 끄거나 generic CPU로 낮춰도 출력이
...
**CI 실측은 이 버전의 커밋이 푸시된 뒤에야 존재한다** — 이 저장소에서 CI 수치는 **항상 후속 커밋**이 쓴다(D85가
`git log`로 세어 확인한 성질이고 가드가 head 항목을 그래서 면제한다). 다음 커밋에서 run 번호와 세 레그를 적는다.
