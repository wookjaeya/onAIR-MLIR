...
# The guest runs detached under setsid+nohup (NOT -daemonize: that makes QEMU
# fork away from any log capture, and this guest has crashed silently more
# than once in this environment -- qemu_stderr.log is the only record when it
# does). The serial console is a UNIX socket (server=on, wait=off) at
# $GUEST_DIR/serial.sock so scripts/73_console.sh can both read and write it
# (needed to drive an emergency-mode shell, etc). A background socat tees the
# socket to $GUEST_DIR/serial.log for grepping. A QMP-style human monitor
# socket is left at $GUEST_DIR/monitor.sock.
set -euo pipefail
...
setsid nohup qemu-system-aarch64 \
  ...
  > qemu_stdout.log 2> qemu_stderr.log < /dev/null &
QEMU_PID=$!
disown
...
disown
echo "qemu-system-aarch64 started: pid $QEMU_PID, smp=$SMP mem=${MEM}MiB, ssh port $SSH_PORT (stderr: $GUEST_DIR/qemu_stderr.log)"
echo "waiting for sshd ..."
for i in $(seq 1 240); do
  if ! kill -0 "$QEMU_PID" 2>/dev/null; then
    echo "qemu-system-aarch64 (pid $QEMU_PID) EXITED while waiting for sshd -- see $GUEST_DIR/qemu_stderr.log:"
    tail -n 40 qemu_stderr.log || true
    exit 2
  fi
...
echo "guest did not come up in 20 min; see $GUEST_DIR/serial.log, $GUEST_DIR/qemu_stderr.log, or scripts/73_console.sh"; exit 1
