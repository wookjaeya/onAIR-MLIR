...
| **정규 MLIR compiler pass** (2026-09-11 추가 확정) | 아래 별도 절 |
...
### 정규 MLIR pass — 이번 논문에서 **하지 않는다** (2026-09-11, 연구 책임자 결정)

R-3의 "강화안"(PassManager에 등록돼 완전한 in-memory module 위에서 돌며 계약을 컴파일러
산출물로 직접 emit하는 **진짜 pass**)은 **이번 논문의 범위에서 제외**한다. E27이 *"결과를 보고
판단한다"*로 남겨 두었던 항목이고, 그 판단이 여기서 내려졌다.

**이 결정이 뜻하는 것과 뜻하지 않는 것을 구분한다** — 이 저장소가 D65에서 고친 유형
(*정정이 산문에만 남았는지 확인하라*)을 여기서 반복하지 않기 위해서다.

| | |
|---|---|
| **뜻한다** | 이번 논문에서 정규 pass를 **구현하지 않는다**. 따라서 *"정규 MLIR pass를 구현했다"*는 앞으로도 쓸 수 없고, 현 구현의 정확한 이름은 여전히 **`--mlir-print-ir-after` 덤프를 다시 읽는 post-processing verifier**다 |
| **뜻하지 않는다** | *"정규 pass를 만들어도 결과가 바뀌지 않음을 실측했다"* — **그런 실험은 없다.** E27·E35가 보인 것은 *현재 비교한 두 정보 수준에서 수치·판정 우위가 관측되지 않았다*까지이고, pass를 만들어 비교한 적은 없다. 이 가드레일은 결정 이후에도 그대로 유지된다 |

**근거**: E27이 R-3의 답을 *"MLIR 수준이 주는 것은 더 정확한 수치가 아니라 독립적인 두 번째
정보원"*으로 좁혔고, v0.31 정오표가 그것을 다시 **가용성**(탐지 이후에 무엇이 남는가)으로 좁혔다.
E35는 같은 계약 범위·같은 조건부 지식에서 **세 수치와 커널 스택이 4/4로 일치**함을 보였다.
즉 pass로 다시 구현하는 것은 **같은 정보원에서 같은 수치를 다른 방법으로 얻는 일**이며,
논문의 중심 주장(**MLIR 기반 계약 추출·연계 방법**)을 바꾸지 않는다. 착수 비용은 크고
논증 이득은 확인되지 않았다.

**후속 연구 항목으로 남긴다.** 제외는 *"가치 없다"*가 아니라 *"이 논문의 축이 아니다"*이며,
R-4(다중 앱 전역 예산)·R-5(시간 축 계약)와 같은 지위다.
