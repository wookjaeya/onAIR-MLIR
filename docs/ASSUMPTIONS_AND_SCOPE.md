...
## 용어 확정 — 7개 구분 (2026-09-10, v0.41/E37)

열한 번째 외부 문서 `RESEARCH_COMPLETION_ACTIONS.md` §3이 요구한 확정이다. 근거와 실측 수치는
`docs/EVIDENCE_v0.41_E37.md` §2에 있다.

| 항목 | 확정된 의미 | 그렇게 읽으면 안 되는 것 |
|---|---|---|
| **예산** | 앱에 **배정한** 계약 대상 메모리 예산 | 시스템 전체 가용 RAM의 자동 탐지 |
| **계약 범위** | `per_call` 버퍼 + 모듈 상주 상수 | 프로세스 RSS나 OBC 전체 메모리 |
| **무조건 정책** | 보수적 상한 `bounded`와 예산의 비교 | 항상 tight한 값 |
| **조건부 정책** | 전제(모듈 이미지 포인터 64 B 정렬)를 **측정으로 확인**했을 때 `per_call`을 적용. opt-in 기본 off, 전제 미성립이면 런타임 생성 전 거부 | 가정해도 되는 최적화 |
| **실행 타깃** | AArch64 QEMU 게스트의 cFS **기능·메모리 관측** 실험 | 실물 비행 하드웨어 검증 |
| **OnAIR** | **공식 플러그인 인터페이스 호환성의 보조 실증** | 계약의 메모리 준수 대상(HAL peak 미측정, 객체 해제 미검증) |
| **MLIR 기여** | **MLIR 기반 계약 추출·연계 방법** | *"MLIR이라야 얻는다"* 또는 *"VMFB-only보다 수치적으로 우월"* |

**계약의 정확한 이름은 그대로다**: partial per-app model-execution memory contract.
