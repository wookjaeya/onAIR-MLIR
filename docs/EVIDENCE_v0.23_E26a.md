...
- **CI 실측**(커밋 `3afdf80`, 워크플로 run 56): `full` **228/228 + 1 SKIP**(PyYAML 미설치) · `without-iree` **126/126 + 13 SKIP** · `stdlib-only` **126/126 + 13 SKIP**. 이 컨테이너와 `full`의 차이 1건은 PyYAML 유무이며, 두 수치를 조건과 함께 병기한다(D34). (E26b의 8건까지 포함된 최종 상태다.)
