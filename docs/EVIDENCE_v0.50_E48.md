...
이 컨테이너 **721/721 → 748/748**(FAIL 0 · SKIP 0). 신규 27건 중 D80 5건은
...
## §6.2 D81 — CI가 잡은 셋째 결함

E48을 마무리하며 CI 로그를 확인하다 찾았다. **E47이 D79를 고치며 넣은 과잉 거부 확인 가드**가 numpy
부재 분기에서 `Result(..., None, "numpy not installed")`를 쓰고 **`skip=True`를 넘기지 않았다**.
`Result.__repr__`는 `ok=None`에 skip이 없으면 **FAIL**을 인쇄하고 집계도 실패로 센다.

결과: **CI 축소 두 레그가 두 커밋 동안 빨간 상태**였다(`4cd26f5`에서 553/554, `08a791b`에서 577/578).
`full` 레그와 이 컨테이너는 numpy가 있어 초록이었으므로 **로컬에서는 원리적으로 보이지 않는 조건**이고,
사람이 아니라 **CI가 잡았다**(D25와 같은 부류).

**D32의 재발이다** — 그때는 `jsonschema` 부재가 15건의 거짓 FAIL을 냈고, 저장소는 **바로 그것 때문에**
`Result`에 `skip` 플래그를 만들었다. 수치·판정 영향은 0이다.

수정은 `skip=True` 추가에 그치지 않고 **구조적 가드**를 신설했다 — **AST**로 `Result(..., None, ...)`
호출을 찾아 `skip` 키워드가 없으면 실패한다. 그 가드의 첫 판은 정규식이라 **자기 패턴 리터럴**을 잡았고
(D77의 형태), AST로 바꾼 뒤에도 **자기 양성 대조**를 잡았다 — D77이 쓴 방식 그대로 **함수 단위 waiver +
건너뛴 줄 원문 노출**로 처리했다. 양성 대조(스캔이 실제로 미표시 None을 본다)도 함께 싣는다:
***발동할 수 없는 검사는 검사가 아니다.***

검증은 **numpy import를 실제로 차단해** 그 케이스가 SKIP으로 나오는 것을 확인했고,
revert 시 1건이 FAIL한다.
...
4. ***부재를 실패로 적지 않기 위해 만든 플래그가 있어도, 쓰지 않으면 없는 것과 같다*** (D81) — 그리고
   그 규율은 **구조로 강제할 수 있다**. 산문으로 적어 둔 교훈은 다음 실험에서 다시 어긴다.
