...
## 8. 적대적 검증 (이 세션, 반박 9묶음 완주 / 2인 검증 0건 — 미실행) + E30b (D56)

E30의 주장을 9개 묶음(provenance · SQUEEZE 변환 · 결정론 · 인터페이스 변경 · 계약 수치 · 두 분기 구조 · fixture
무결성 · 범위·주장 · 확장의 과잉 거부)으로 나눠 각각 독립 반박 에이전트에 맡겼다.

**실행된 것과 실행되지 않은 것을 먼저 적는다.** 계획은 E20 이후의 관례대로 "note가 아닌 finding마다 2인이 따로
재현·판정"이었고, **반박 9묶음은 전부 완주했으나 2인 검증 에이전트는 한 건도 실행되지 않았다** — 두 번의 실행
모두 세션 한도(`You've hit your session limit`)로 전부 실패했다(1차 25개 중 7 완주·18 실패, 재시도 39개 중 9
완주·30 실패). 따라서 **이 절의 어떤 판정도 "2인 검증을 통과했다"는 뜻이 아니다.** 아래 수정 3건(D56 계열)은
반박 에이전트의 재현을 **이 세션이 직접 다시 재현하고 revert-and-confirm-fail로 확인한** 것이며, 그 확인의
주체는 독립 검증자가 아니라 구현자 자신이다. 이 한계는 v0.9.1·E26d에서 두 번 정정한 유형(실행하지 않은 시험을
실행한 것처럼 적는 것)이라 결과가 유리한 쪽이어도 그대로 남긴다.
...
### 8.3 나머지 두 묶음 — 범위·주장, 확장의 과잉 거부

| 묶음 | 결과 | 조치 |
|---|---|---|
| 범위·주장 | 유지(반박 실패) + 실물 결함 2건 | 분석서 §4·§5.4의 산출물이 전부 실재·실질적임을 확인(원본을 upstream에서 직접 재취득해 대조, 조건 7이 NOT DONE으로 정확히 기록됨, §8 do-not-do 목록을 커밋 이력으로 확인). **그러나 `p1_tflite_inventory.py`에서 medium 2건**: (a) SqueezeOptions 테이블이 없는 TFLite-유효 SQUEEZE에서 `AttributeError`로 **크래시** — E30b가 변환기에서 방금 고치고 결함 원장에 "조치 완료"로 적은 바로 그 조건이 **형제 도구에는 남아 있었다**; (b) 음수 `squeeze_dims`에서 C1·C4를 **위반으로 보고** — 같은 파일을 변환기는 정상 수용하므로, 감사 도구가 자기가 감사하는 변환을 반박하고 있었다. 둘 다 비행 모델은 무관(dims [1,2], 옵션 테이블 있음). **수정**: 인벤토리가 변환기의 `normalize_squeeze_dims`·`check_squeeze_conditions`를 **직접 import**해 두 도구가 구조적으로 갈라질 수 없게 했다. 다중 subgraph도 sibling note에서 `inventory_complete: false` 상태 필드로 승격 |
| 확장의 과잉 거부 | **반박 성공(refuted=true)** | 아래 §8.4 |

### 8.4 C5 자신이 과잉 거부였다 — 축 위치 논증을 시뮬레이션으로 교체

E30b가 D56을 고치며 도입한 C5는 **"남는 축이 레이아웃 순열 아래에서 상대 순서를 유지해야 한다"**는 축 위치
논증이었다. 반박 에이전트가 그 논증이 **양방향으로 틀렸음**을 보였고, 이 세션이 저장소 도구로 직접 재현했다.

- **과잉 거부(재현)**: 재정렬되는 남는 축이 **전부 extent 1**이면 데이터는 움직이지 않는다. `[2,1,1,1]`에
  `squeeze_dims [1]`은 C1–C4를 통과하고 방출도 정확한데(IREE 출력 = `np.squeeze`, `max_abs_diff 0.0`) C5가
  *"would reorder data"*라는 **사실이 아닌 이유로** 거부했다. 에이전트의 오라클 대조로는 C1–C4를 통과하는
  레이아웃 태그 rank-4 81개 형상 중 **7개**가 이 부류다(같은 스윕에서 fail-open은 0).
- **명백한 수정이 곧 fail-open**: "extent>1인 남는 축만 순서를 지키면 된다"로 완화하면, `[1,2,1,1]`에
  `squeeze_dims [0]`이 그 규칙을 만족하면서 **선언된 출력 형상 `[2,1,1]`과 ONNX가 추론하는 `[1,2,1]`이
  어긋난다**(평탄화 값은 우연히 일치). 즉 한 번 더 고친 논증도 틀린다.

**그래서 논증을 다시 고치지 않고 질문을 직접 계산하게 했다.** C5는 이제 모델 자신의 입력 형상으로 라벨 배열을
만들어 레이아웃 순열과 재색인된 squeeze를 **방출될 ONNX 그대로 시뮬레이션**하고, 그 결과가 TFLite 자신의 기준
(`np.squeeze`, 변환 전 입력)과 **형상과 값 둘 다** 같을 때만 통과시킨다. 이것은 `e30b_squeeze_probe.py`가
`--iree`로 확인하는 바로 그 등식을 컴파일러 없이 계산하는 것이라, C5가 **맞는 답을 틀린 이유로** 낼 여지가
사라진다. 배열은 모델 형상 크기라(SmartCam 최대 1,280 원소) 비용은 무시할 수준이다.

결과(생성기에 두 경계 사례를 신설해 고정):

| 사례 | 이전 C5 | 현재 C5 | IREE 실측 |
|---|---|---|---|
| `sq_c5_extent1_reorder` `[2,1,1,1]` dims `[1]` | **거부(과잉)** | 통과 | 두 방출 모드 다 `max_abs_diff 0.0` |
| `sq_c5_shape_mismatch` `[1,2,1,1]` dims `[0]` | 거부 | **거부 유지** | (거부되어 방출 없음) |
| SmartCam `[1,1,1,1280]` dims `[1,2]` | 통과 | 통과 | ONNX **바이트 불변** `79c57abc…` |
| `sq_H` · `sq_H77` · `sq_N` · `sq_identity` | 거부 | 거부 유지 | (D56의 원래 사례) |

비행 모델의 ONNX·linalg·vmfb·계약은 **전부 불변**이고 변환 매니페스트의 감사 필드만 바뀌었다
(`kept_axes_positions_after_layout` → `C5_decided_by`). 회귀 시험은 두 경계 사례와 **인벤토리↔변환기 판정
일치** 3건을 새로 고정한다. 이 컨테이너 **371/371 → 378/378**.
**CI 실측**(커밋 `e4e60c3`, run 121, 3레그 전부 success): `full` **377/377 + 1 SKIP**(PyYAML) ·
`without-iree` **241/241 + 22 SKIP** · `stdlib-only` **241/241 + 22 SKIP** — 컨테이너 378/378과 `full`의
차이 1건은 PyYAML 유무다(D34). 축소 레그의 총 검사 수가 컨테이너보다 적은 것은 변환기 패키지가 없으면
`e30b_squeeze_extension_cases`가 **SKIP 2건을 남기고 조기 반환**하기 때문이며(사례를 하나씩 SKIP으로
나열하지 않는다), 그래서 D57 신규 7건 중 5건은 그 레그에 아예 나타나지 않는다.

**교훈**: E30b의 교훈이 *"형상·원소 수·dtype 보존 ≠ 데이터 순서 보존"*이었다면, 그 수정 자체가 남긴 교훈은
***"경계 조건을 논증으로 좁히면 논증이 틀린 만큼 틀린다 — 계산할 수 있는 것은 계산하라."*** 두 번 연속으로
같은 자리에서 틀린 뒤에야 시뮬레이션으로 바꿨다.
