...
## 9. 정오표 (E24, 외부 검토 v0.18-후속 S5 — 명칭. 수치·판정 변경 없음)

이 문서의 제목과 §0이 이 작업을 **"정규 MLIR pass 2단계"**라 부른다. 그 명칭은 부정확하다 —
`harness/mlir_alloc_walk.py`는 IREE/MLIR의 `PassManager`에 등록되어 완전한 in-memory module
위에서 실행되는 pass가 아니라, `--mlir-print-ir-after`가 남긴 **텍스트 덤프를 별도 프로세스에서
다시 읽는 "MLIR API 기반 구조적 post-processing verifier"**다. 정본 정정은
`docs/EVIDENCE_v0.13_E18.md` §7(E23/F4에서 신설)이며, 이 문서는 그 1단계 결과물을 파이프라인에
결선한 2단계다.

**이 정정이 바꾸지 않는 것**: §3의 14/14 일치, §4의 저하 동작 확인, §6의 판정, §7의 재현 절차
어느 것도 달라지지 않는다. 이 문서가 측정한 것은 전부 그대로 사실이다 — 잘못된 것은 그 결과를
부르는 이름뿐이다. 바꾸는 것은 **"pass 단계가 완료됐다"고 읽힐 여지**뿐이며, 진짜 pass(계약을
컴파일러 호출의 직접 산출물로 emit하는 구현)는 이 문서 시점에도, E24 시점에도 여전히 미착수다.

E24가 함께 정정한 것: `README.md`와 `CLAUDE.md`의 살아있는 서술(항목명·저장소 지도)에서 이
명칭을 직접 고쳤다. 이 EVIDENCE 본문은 작업 규율 5에 따라 고쳐쓰지 않고 이 절만 덧붙인다.
