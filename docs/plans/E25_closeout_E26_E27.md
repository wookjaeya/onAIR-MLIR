# 계획: E25 주장 범위 정정 → E25b → E26 → E27

> **진행 상태(2026-09-09 갱신)**
> - **Phase A 완료** — `bee2505` (v0.22.1): E25 주장 범위 5건 정정, D45·D46.
> - **Phase B 완료** — 같은 커밋 (E25b): 쌍별 판정 규칙, 과잉 거부 재현·수정, 회귀 12건.
> - **Phase C-3(측정 전 필수) 완료** — `e4c6371` (v0.22.2): 계측 분리.
> - **계획 밖에서 추가된 것**: 벤치마크 구성 지침(`docs/reviews/BENCHMARK_PLAN_REFERENCE_BASED.md`)
>   도입 조사 중 재현된 과잉 거부 2건을 수정 — `b9548a4` (v0.23/E26a, D47),
>   `0dea23d` (v0.24/E26b, D48).
> - **E26 본 측정의 사전 고정 기준은 별도 문서**: `docs/plans/E26_boundary_utility.md`.
>   벤치마크 포트폴리오와 실측 검증 결과가 그쪽에 정리돼 있다.
> 아래 본문은 작성 당시 그대로 두며(이력), 완료 항목은 위 목록이 정본이다.
